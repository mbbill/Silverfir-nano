...
- 2025-08-13 (db80d0cd) rationale: the handler-carried register bank is sized to
  ...
  line across the tail-chained handlers (diff).
...
- 2025-08-14 (01c8ee79) rationale: the hot operand window is a fixed 3-lane
  ...
  spill/fill to the memory stack; this keeps hot paths register-only with branchy
  code that predicts well and avoids indexed addressing that blocks SROA, and
  depth is held as u64 to avoid byte-width extends on some ABIs (diff).
...
- 2025-08-16 (d748e227) pitfall: the window's spill/fill discipline was
  ...
  spill slots, which a subsequent flush could write back spuriously (diff).
...
- 2025-08-16 (38b204bd) pitfall: on return from a callee the caller's live top
  ...
  results, replacing the callee's args in place with its results (diff).
...
- 2025-08-16 (10c1c487) replaced [[value-stack-self-tracked-sp-offset]]: the stack
  ...
  stack_size parameter removes the dual-bookkeeping and its sync hazard (diff).
...
- 2025-08-17 (912cc440) replaced by [[pure-sp-memory-stack]]: the by-pointer Regs
  ...
  carried in a heap-shaped register struct (diff).
