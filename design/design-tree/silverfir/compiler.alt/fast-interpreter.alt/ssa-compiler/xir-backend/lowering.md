...
- 2025-10-12 (a5dc69b9) pitfall: comparison operators return i32 but must lower to
  ...
  comparison for i64/f32/f64 operands (diff).
...
- 2025-11-12 (64d36bb5) pitfall: a unary op's type field is what keys its
  ...
  that reads only 32 bits of the 64-bit operand (diff).
...
- 2025-10-10 (e6646469) pitfall: the lowering emits a phi's predecessor copies
  ...
  a phi-bearing block must be a materialized branch terminator (diff).
...
- 2025-10-16 (a830c8a8) pitfall: br_table lowering emitted a jump-over-the-stubs
  ...
  location, so the stub-skip jump is removed entirely (diff).
...
- 2025-11-12 (0c84b408) pitfall: register allocation can land both arms of a
  ...
  select, since the condition is then irrelevant (diff).
...
- 2025-11-01 (2646713a) statement: each SSA instruction reports its used and
  ...
  liveness consume (diff).

- 2025-10-25 (bd23526f) rationale: br_if originally branched on a ZERO condition
  (justified as 'easier code generation') and the lowerer compensated by swapping
  then/else targets; this was abandoned for the native WebAssembly polarity
  (branch on NON-ZERO to then_target, unconditional br to else_target), producing
  equivalent control flow at the same instruction count without the target-swapping
  inversion (diff).
...
- 2025-10-12 (2bee2d7a) replaced [[br-table-direct-targets]]: a single jump table
  ...
  performs that target's phi stores before jumping to the real block (diff).
...
- 2025-10-16 (d1307ec8) replaced [[shuffle-to-fixed-slots]]: forcing operands into
  ...
  put and no shuffle instructions are emitted (diff).
...
- 2025-10-24 (010b53d5) replaced [[indirect-target-in-window]]: a call is control
  ...
  side-effect op (Sig_0_0) identical in shape to direct call (diff).
...
- 2025-11-09 (d04cbd44) revived: the LIR migration moved phi elimination upstream
  ...
  the default — dropping the per-target stubs (diff).
