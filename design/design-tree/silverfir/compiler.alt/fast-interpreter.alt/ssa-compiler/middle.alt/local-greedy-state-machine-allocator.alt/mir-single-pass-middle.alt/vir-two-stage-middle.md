...
- 2025-10-18 (50b75e8e) rationale: register allocation runs once on the SSA→VIR
  boundary so every backend shares the result — recomputing live-range analysis
  (expensive) in each backend was rejected — and virtual registers are abstract
  ...
  constraints; allocating SSA straight to physical would couple the IR to one
  backend (diff).

- 2025-10-18 (50b75e8e) rationale: fusion is carried as frontend-emitted hints (a
  pattern plus the instruction range it covers) over the VIR rather than as fused
  VInst opcodes (which would bloat and de-portabilize the ISA) or per-backend
  pattern matching (which duplicates the matcher in every backend) (diff).
...
- 2025-11-01 (95cfadd8) rationale: the interference-graph representation is chosen
  ...
  switch to sparse adjacency lists to bound memory (diff).
...
- 2025-11-01 (fa5c0686) measurement: graph coloring reuses vregs that
  ...
  retained as the --no-regalloc debugging fallback (diff).
...
- 2025-11-01 (2646713a) statement: the CFG-based dataflow liveness adopts the
  ...
  dependency commitment, not a reimplementation (diff).
...
- 2025-10-26 (760b71b2) rationale: liveness and the interference graph are stored
  ...
  gives hashing-free O(1) insert/lookup at the cost of O(n²) memory (diff).
...
- 2025-10-26 (adb91990) rationale: after VIR lowering, per-VReg metadata (use
  ...
  purely structural heuristics ([[../../../xir-backend]]) (diff).
...
- 2025-11-01 (7d2d58df) statement: liveness is computed by backward dataflow to
  ...
  predecessor exits and parameters live at entry (diff).
...
- 2025-11-01 (abd92456) pitfall: the interference builder originally excluded a
  ...
  interferes with all of its sources unconditionally (diff).
...
- 2025-11-01 (70957244) pitfall: two phi destinations in the same block must
  ...
  interference edge is required (diff).
...
- 2025-10-18 (9ff77d1b) replaced [[vir-two-stage-middle.alt/identity-window-single-stage-lowering]]:
  ...
  and the virtual-register IR serves both the interpreter and a future JIT (diff).
...
- 2025-10-23 (a9655e3e) replaced [[vir-two-stage-middle.alt/collapsed-extension-nullable-vir]]:
  ...
  correct runtime behavior and pushes any fusion to the backend (diff).
...
- 2025-11-04 (e9790144) replaced by [[../mir-single-pass-middle]]: the old middle
  ...
  defers all allocation to one target-aware pass (diff).
