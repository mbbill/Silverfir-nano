...
- 2025-09-17 (aed2ff42) rationale: compute is register-based, not stack-based —
  ...
  store and only the top few slots are register-resident (diff).
...
- 2026-02-08 (89f91c70) rationale: the per-(operation, type, register
  ...
  generated handler set finite (diff).
...
- 2026-02-08 (da03882b) rationale: the hot handlers are written in C rather than
  ...
  worked only on Windows with a special toolchain; keeping the hot handlers in C
  avoids needing cross-language LTO while still getting C-only LTO to perform the
  guard-check (bounds/zero-check) elimination the handlers depend on — the same
  rationale behind the fast backend's `handlers_c/` (diff).
