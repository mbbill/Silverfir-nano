- RefHandle is a pointer-width tagged index whose only tags are HOST_TAG and
...
- 2026-04-16 (9ff58dcd) replaced by [[ref-handle]]: the two-way host/extern tag
  ...
  pool_payload bit inside the payload so one RefHandle distinguishes null,
  special, host, extern, and pooled (i31/GC) references and routes pooled handles
  to the per-store ref registry (diff).
