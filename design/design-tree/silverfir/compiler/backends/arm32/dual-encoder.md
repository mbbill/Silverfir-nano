...
- 2026-04-14 (1f5c0da9) rationale: A32 dispatches a br_table in O(1) with a
  ...
  a non-zero shift) so it emits an O(N) cmp+beq chain, accepted because wasm
  br_tables are almost always small (diff).
...
- 2026-04-14 (1f5c0da9) pitfall: Thumb-2 has no per-instruction condition field;
  ...
  trailing NOP to stay 4-byte-slot-aligned (diff).
...
- 2026-04-22 (05464604) pitfall: a Thumb-2 PUSH/POP register-list encoder must
  ...
  lenient qemu but faults under a hardened decoder / real M33 silicon (diff).
