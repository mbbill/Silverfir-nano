...
- 2026-03-23 (c06c102a) rationale: functions are pushed onto the next 16 KB
  page only when the padding needed is at most 1 KB — a function starting far
  from the boundary already has its hot early blocks on the current page so the
  tail crossing is cheap, and capping padding at 1 KB keeps overhead under 1%
  on large (~800-function) modules while still fixing roughly a third of page
  crossings; the goal is reduced iTLB pressure on hot entry blocks without
  inflating total code size (diff).
...
- 2026-03-23 (6b4ba56e) rationale: the NOP/pad encoding is architecture-specific
  ...
  conditional page-crossing push (author).
