...
//!
//! Syscalls operate on the context captured by their import callbacks.
...
impl WasiCtx {
    fn dir_fd_state(&self, fd: i32) -> Result<(PathBuf, u64, u64), i32> {
        self.with_ctx(|ctx| {
    ...
    pub(super) fn args_sizes_get(
        ...
        let (argc, buf_size) = self.with_ctx(|ctx| {
    ...
    pub(super) fn args_get(
        ...
        let argv: collections::Vec<String> = self.with_ctx(|ctx| ctx.args.clone());
    ...
    pub(super) fn environ_sizes_get(
        ...
        let (count, size) = self.with_ctx(|ctx| {
    ...
    pub(super) fn environ_get(
        ...
        let env: collections::Vec<(String, String)> = self.with_ctx(|ctx| ctx.env.clone());
    ...
    pub(super) fn fd_write(
        ...
        match fd {
            1 | 2 => {
                // stdout / stderr
                let is_closed = self.with_ctx(|ctx| ctx.closed_stdio.contains(&fd));
                if is_closed {
            ...
            _ => {
                ...
                let errno = self.with_ctx_mut(|ctx| match ctx.fds.get_mut(&fd) {
                    Some(FdEntry::File {
                        file,
                        rights_base,
                        fdflags,
    ...
    pub(super) fn fd_read(
        ...
        match fd {
            0 => {
                ...
                let is_closed = self.with_ctx(|ctx| ctx.closed_stdio.contains(&0));
                if is_closed {
                    results[0] = Value::I32(ERRNO_BADF);
                    return Ok(());
                }
    ...
    pub(super) fn fd_close(
        ...
        let errno = self.with_ctx_mut(|ctx| {
            // stdio
            if (0..=2).contains(&fd) {
    ...
    pub(super) fn fd_seek(
        ...
        let result = self.with_ctx_mut(|ctx| match ctx.fds.get_mut(&fd) {
            Some(FdEntry::File {
                file,
                host_path,
                rights_base,
    ...
    pub(super) fn fd_fdstat_get(
        ...
        let result: Result<(u8, u16, u64, u64), i32> = self.with_ctx(|ctx| {
            match fd {
                0 => {
                    // stdin
    ...
    pub(super) fn path_open(
        ...
        let (base_host_path, base_allowed_base, base_allowed_inh) = match self.dir_fd_state(fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
        ...
        if follow_kind.is_some_and(|kind| kind.is_dir) {
            ...
            let allowed_dir_base = RIGHT_FD_READDIR
                | RIGHT_FD_FILESTAT_GET
                | RIGHT_FD_FILESTAT_SET_TIMES
                | PATH_RIGHTS;
            let dir_rights = rights_base & allowed_dir_base;
            let new_fd = self.with_ctx_mut(|ctx| {
    ...
    pub(super) fn fd_filestat_set_times(
        ...
        let errno = self.with_ctx(|ctx| match ctx.fds.get(&fd) {
            Some(FdEntry::File {
                file, rights_base, ..
            }) => {
    ...
    pub(super) fn fd_readdir(
        ...
        let host_dir = match self.dir_fd_state(fd) {
            Ok((host, _, _)) => host,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
    ...
    pub(super) fn path_create_directory(
        ...
        let (base_host, _, _) = match self.dir_fd_state(fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
    ...
    pub(super) fn path_filestat_set_times(
        ...
        let mem = get_mem(caller)?;
        let (base_host, base_rights, _) = match self.dir_fd_state(fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
    ...
    pub(super) fn path_rename(
        ...
        let (base_old, _, _) = match self.dir_fd_state(fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
        ...
        let (base_new, _, _) = match self.dir_fd_state(new_fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
    ...
    pub(super) fn path_link(
        ...
        let (base_old, _, _) = match self.dir_fd_state(old_fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
    ...
    pub(super) fn path_symlink(
        ...
        let (base, _, _) = match self.dir_fd_state(fd) {
            Ok(v) => v,
            Err(errno) => {
                results[0] = Value::I32(errno);
                return Ok(());
