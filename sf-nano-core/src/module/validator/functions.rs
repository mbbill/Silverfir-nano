...
impl<'a> FunctionValidator<'a> {
    ...
    fn on_op(
        ...
        match op {
            ...
            BR_TABLE => {
                ...
                labels
                    ...
                    .try_for_each(|&label| {
                        ...
                        let popped = self.context.pop_vals_with_types(&label_types)?;
            ...
            REF_EQ => {
                self.context.pop_val(Some(ValueType::eqref()))?;
                self.context.pop_val(Some(ValueType::eqref()))?;
                self.context.push_val(I32)?;
                Ok(())
            }
...
impl Context {
    ...
    fn pop_vals(&mut self, expected_vals: &[ValueType]) -> Result<(), WasmError> {
    ...
    // br_table restores the actual operand types between labels, including
    // polymorphic Unknown values; other callers only need type checking.
    fn pop_vals_with_types(
        &mut self,
