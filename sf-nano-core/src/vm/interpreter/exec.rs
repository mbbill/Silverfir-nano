...
impl<'m> InterpInstance<'m> {
    ...
    fn enable_native_dispatch(&mut self) -> Result<(), WasmError> {
        ...
        let callee_info: Vec<Option<(u64, u64)>> = self
            .funcs
            .iter()
            .enumerate()
            .map(|(i, f)| match (f, &linked[i]) {
                (Some(f), Some(lf)) => {
                    ...
                    Some((lf.cells.as_ptr() as u64, packed))
                }
                _ => None,
            })
            .collect();
        for (i, lf) in linked.iter_mut().enumerate() {
            ...
            for (k, ins) in f.code.iter().enumerate() {
                if ins.op == Op::Call {
                    if let Some(Some((cells, packed))) = callee_info.get(ins.a as usize) {
                        lf.cells[k] = DCell {
                            h: call_h,
                            a: *cells,
                            b: ins.b * 8,
    ...
    fn native_step(
        ...
        let (enter, exit_cell, cells_base) = match info {
            Some(x) => x,
