...
mod tests {
    ...
    #[test]
    fn apple_backedge_branches_preserve_taken_fallthrough_and_dispatch_count() {
        let (bin, _) = instantiate(
            r#"(module
                (global $barrier i32 (i32.const 0))
                (func (export "br_if") (param $flag i32) (result i32)
                    (loop $again
                        (local.get $flag)
                        (i32.const 1)
                        (i32.sub)
                        ...
                        (br_if $again))
                    (local.get $flag))
                (func (export "br_if_not") (param $flag i32) (result i32)
                    (loop $again
                        (local.get $flag)
                        (i32.const 1)
                        (i32.add)
        ...
        let module = Module::new("apple-backedge-branches", &bin).expect("module");
        let mut inst = InterpInstance::new(
            &crate::vm::engine::Engine::with_defaults(),
            module,
            None,
        ...
        for (function_index, export, op, argument, expected) in [
            (0, "br_if", Op::BrIf, 2, 0),
            (1, "br_if_not", Op::BrIfNot, u32::MAX as u64, 1),
        ] {
            let function = inst.functions()[function_index]
                .as_ref()
                .expect("defined function");
            let branch_index = function
                .code
                .iter()
                .position(|ins| ins.op == op)
                .expect("requested conditional branch");
            let target = function.code[branch_index].c as usize;
            assert!(target <= branch_index, "test branch must be a backedge");
            let expected_dispatches =
                function.code.len() as u64 + (branch_index - target + 1) as u64;
            let (cell, _, _) = linked_cell_for_op(&inst, function_index, op);
            let native = inst.native.as_ref().expect("native state");
            assert!(
                native
                    .engine
            ...
            let func = inst.find_export(export).expect("export");
            let mut result = [u64::MAX];
            inst.invoke(func, &[argument], &mut result).expect("invoke");
            assert_eq!(result[0], expected, "{export} taken then fallthrough");
            if inst.dispatch_counting_enabled() {
                assert_eq!(
                    inst.dispatch_count() - before,
                    expected_dispatches,
                    "{export} must count each executed dispatch exactly once"
                );
            }
        }
    }
