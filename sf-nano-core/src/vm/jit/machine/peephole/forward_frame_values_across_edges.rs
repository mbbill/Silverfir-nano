...
use crate::vm::jit::machine::machine_ir::{
    ...
    MachineStorageType, MachineTerminator, MachineValue, MACHINE_FIXED_REG_COUNT, MACHINE_FP_REG,
    MACHINE_MEM0_BASE_REG,
};

use super::helpers::{inst_defines, inst_uses_value, store_may_alias};
use super::hoist_loop_address_bases::{
    block_mentions_reg, visit_edges, visit_edges_mut, LoopGraph,
};
            ...
            let carry = existing.or_else(|| {
                    ...
                    })
                {
                    ...
                    (MACHINE_FIXED_REG_COUNT..gp_end)
                        .map(MachineReg)
                        .find(|&reg| !block_mentions_reg(&blocks[target], reg))
                }
            });
...
mod tests {
    ...
    fn optimize(blocks: &mut [MachineBlock], entry: u32) {
        ...
        let mut ctx = BlockOptCtx::new(BackendConfig::new(8, 7, 0, 0));
