...
fn try_cache_word(
    ...
    // Only blocks with new carried-word copies need the alias reassignment.
    // Keep its scratch allocation and instruction walks out of other blocks.
    for index in rewritten {
        super::reuse_gp_temporaries::reuse_gp_temporaries(
            core::slice::from_mut(&mut blocks[index]),
            config,
        );
    }
}
