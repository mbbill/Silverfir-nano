//! Reassign registers inside bounded integer fragments with frame publication.
//!
...
//! Aligned whole-word native-frame stores keep their original order and values.
//! Loads, guest memory, calls, traps and lowering scratch remain boundaries.

use crate::collections;
use crate::vm::jit::backend::BackendConfig;
use crate::vm::jit::machine::machine_ir::{
    MachineBlock, MachineBranchCond, MachineInst, MachineInstKind, MachineIntBinaryOp,
...
pub(super) fn reuse_gp_temporaries(blocks: &mut [MachineBlock], config: BackendConfig) {
    ...
    for block in blocks {
        ...
        // A final whole-word cache publication already provides the branch
        // value. Use it directly so the temporary need not stay live solely
        // for the condition. Edge bindings and ownership remain unchanged.
        if let (
            Some(MachineInst {
                kind:
            ...
            MachineTerminator::Branch {
                cond: MachineBranchCond::Value(MachineValue::Reg(condition)),
                ..
            },
        ) = (block.ops.last(), &mut block.terminator)
        {
            if *condition == *src
                && dst.0 >= BackendConfig::FIXED
                && dst.0 < end
                && src.0 >= BackendConfig::FIXED
                && src.0 < end
            {
                *condition = *dst;
            }
...
fn eligible(kind: &MachineInstKind, end: u16) -> bool {
    ...
    {
        // Canonical frame slots are internal, already allocated and addressed
        // by an unchanged fixed FP. This pass neither moves nor deletes stores;
        // it only substitutes the physical lane holding each published value.
        return addr.base == MACHINE_FP_REG
            && addr.offset >= 0
            && addr.offset % 8 == 0
            && match src {
                MachineValue::Reg(reg) => dynamic(reg),
                MachineValue::Imm64(_) => true,
...
fn destination(kind: &mut MachineInstKind) -> &mut MachineReg {
    match kind {
        ...
        | MachineInstKind::IntCompare { dst, .. }
        | MachineInstKind::TestBits { dst, .. } => dst,
        _ => unreachable!("validated integer result"),
    }
}
...
fn inputs(kind: &mut MachineInstKind, mut visit: impl FnMut(&mut MachineReg)) {
    ...
    match kind {
        MachineInstKind::Move { src, .. } | MachineInstKind::Store { src, .. } => {
            value(src, &mut visit)
        }
        ...
        _ => unreachable!("validated integer or frame-publication fragment"),
    }
}
...
fn rewrite(original: &[MachineInst], live: u64, end: u16, scratch: &mut Scratch) -> bool {
    ...
    for original_inst in original {
        ...
        let dst = if matches!(inst.kind, MachineInstKind::Store { .. }) {
            None
        } else {
    ...
    for (position, node) in scratch.nodes.iter().enumerate() {
        ...
        let preferred = [
            scratch.values[result].output_hint,
            first_input,
            Some(original_dst),
        ];
        ...
        *destination(&mut inst.kind) = dst;
        ...
        scratch.physical[usize::from(dst.0)] = Some(result);
        scratch.values[result].reg = Some(dst);
...
mod tests {
    ...
    #[test]
    fn carries_updates_in_place_without_changing_frame_publication() {
        let store = |offset, src| MachineInst {
            kind: MachineInstKind::Store {
                ty: MachineStorageType::GpWord,
                addr: MachineAddr {
                    base: MACHINE_FP_REG,
        ...
        let original = collections::vec![
            store(16, 10),
            mv(7, 10, MachineRegOwner::LinearValue),
            binary(
                MachineIntWidth::I32,
                MachineIntBinaryOp::Add,
                7,
            ...
            store(16, 7),
            mv(10, 7, MachineRegOwner::CachedCell),
            mv(8, 10, MachineRegOwner::LinearValue),
            binary(
                MachineIntWidth::I64,
                MachineIntBinaryOp::Add,
                8,
                8,
                MachineValue::Imm64(9)
            ),
            store(24, 8),
            mv(10, 8, MachineRegOwner::CachedCell),
        ];
        assert!(original.iter().all(|inst| eligible(&inst.kind, 11)));
        let mut scratch = Scratch::default();
        let live = bit(MachineReg(10));
        assert!(rewrite(&original, live, 11, &mut scratch));
        assert!(!scratch.output.iter().any(|inst| real_copy(&inst.kind)));
        for seed in [0, 1, u32::MAX as u64, u64::MAX, 0x1234_ffff_8765_4321] {
            verify(&original, &scratch.output, live, [seed; 16]);
        }
    }

    #[test]
    fn branch_uses_the_published_counter_without_a_second_live_alias() {
        let mut block = MachineBlock {
            id: MachineBlockId(0),
            params: collections::Vec::new(),
            ops: collections::vec![
            ...
            terminator: MachineTerminator::Branch {
                ...
                then_edge: MachineEdge {
                    ...
                    args: collections::vec![MachineValue::Reg(MachineReg(10))],
                },
        ...
        assert!(!block.ops.iter().any(|inst| real_copy(&inst.kind)));
        for seed in [0, 1, 2, u64::MAX, 1u64 << 32] {
            let regs = [seed; 16];
            verify(&before.ops, &block.ops, bit(MachineReg(10)), regs);
            let edge_values = |block: &MachineBlock| {
                let (regs, _) = execute(&block.ops, regs);
                ...
                let edge = if operand(&regs, *cond) != 0 {
                    then_edge
                } else {
                    else_edge
                };
                (
                    ...
                    edge.args
                        .iter()
                        .map(|&value| operand(&regs, value))
                        .collect::<collections::Vec<_>>(),
                )
            };
            assert_eq!(edge_values(&before), edge_values(&block));
        }
    }

    #[test]
    fn frame_publication_excludes_other_address_spaces_and_partial_words() {
        for (base, offset, width) in [
            (MachineReg(4), 16, MachineMemWidth::U64),
            (MACHINE_FP_REG, -8, MachineMemWidth::U64),
            (MACHINE_FP_REG, 4, MachineMemWidth::U64),
            (MACHINE_FP_REG, 16, MachineMemWidth::U32),
        ] {
            let kind = MachineInstKind::Store {
                ty: MachineStorageType::GpWord,
                addr: MachineAddr { base, offset },
                width,
                src: MachineValue::Reg(MachineReg(6)),
            };
            assert!(!eligible(&kind, 11));
        }
    }
        ...
        for _ in 0..512 {
            ...
            for _ in 0..24 {
                ...
                let kind = match next() % 9 {
                    ...
                    4 => MachineInstKind::Store {
                        ty: MachineStorageType::GpWord,
                        addr: MachineAddr {
                            base: MACHINE_FP_REG,
                            offset: (next() % 3 * 8) as i32,
                        },
                        width: MachineMemWidth::U64,
                        src: rhs,
                    },
