...
use crate::{
    ...
    vm::{
        ...
        jit::wasm::primitive_op::{self, PrimitiveOpKind},
    },
};
...
mod tests {
    ...
    use crate::vm::{
        jit::middle::frame::{FrameSlot, FrameSpan},
        jit::middle::ssa_ir::{
            ir::{
    ...
    fn folds_single_use_const_into_value_operand() {
        ...
        let absorbed_const = add_inst.args.iter().any(|operand| {
            matches!(
                operand.decode(),
                crate::vm::jit::middle::ssa_ir::ir::DecodedOperand::Const(idx)
    ...
    fn folds_zero_value_operand_into_select() {
        ...
        assert!(matches!(
            ...
            crate::vm::jit::middle::ssa_ir::ir::DecodedOperand::Const(index)
