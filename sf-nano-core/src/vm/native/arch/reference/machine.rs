...
use crate::{
    ...
    vm::{
        ...
        native::{
            ...
            code::NativeCode,
            ir::{NativeBlockId, NativeEdge, NativeInst, NativeInstKind, NativeProgram, NativeTerminator},
...
impl<'a> ReferenceMachine<'a> {
    ...
    fn read_values(&self, values: &[NativeValue]) -> Result<Vec<u64>, WasmError> {
        values.iter().copied().map(|value| self.read_value(value)).collect()
    }
    ...
    fn read_place(&self, place: NativePlace) -> Result<u64, WasmError> {
        match place {
            NativePlace::Location(NativeLocation::Ctx) => Ok(self.ctx as *const NativeContext as u64),
    ...
    fn write_place(&mut self, place: NativePlace, value: u64) -> Result<(), WasmError> {
        match place {
            NativePlace::Location(NativeLocation::Ctx) | NativePlace::Location(NativeLocation::Fp) => {
    ...
    fn eval_leaf(&mut self, op: &LirLeafOp, args: &[u64]) -> Result<Vec<u64>, WasmError> {
        ...
        let out = match op {
            ...
            I32Shl => alloc::vec![u64::from(arg_u32(args, 0)?.wrapping_shl(arg_u32(args, 1)? & 31))],
            I32ShrS => alloc::vec![u64::from((arg_i32(args, 0)? >> (arg_u32(args, 1)? & 31)) as u32)],
            I32ShrU => alloc::vec![u64::from(arg_u32(args, 0)? >> (arg_u32(args, 1)? & 31))],
            I32Rotl => alloc::vec![u64::from(arg_u32(args, 0)?.rotate_left(arg_u32(args, 1)? & 31))],
            I32Rotr => alloc::vec![u64::from(arg_u32(args, 0)?.rotate_right(arg_u32(args, 1)? & 31))],
            ...
            I64ShrS => alloc::vec![((arg_i64(args, 0)? >> ((arg_u64(args, 1)? & 63) as u32)) as u64)],
            ...
            F32Add => alloc::vec![f32_bits(as_f32(arg_u32(args, 0)?) + as_f32(arg_u32(args, 1)?))],
            F32Sub => alloc::vec![f32_bits(as_f32(arg_u32(args, 0)?) - as_f32(arg_u32(args, 1)?))],
            F32Mul => alloc::vec![f32_bits(as_f32(arg_u32(args, 0)?) * as_f32(arg_u32(args, 1)?))],
            F32Div => alloc::vec![f32_bits(as_f32(arg_u32(args, 0)?) / as_f32(arg_u32(args, 1)?))],
            ...
            F32Copysign => alloc::vec![f32_bits(as_f32(arg_u32(args, 0)?).copysign(as_f32(arg_u32(args, 1)?)))],
            F64Add => alloc::vec![f64_bits(as_f64(arg_u64(args, 0)?) + as_f64(arg_u64(args, 1)?))],
            F64Sub => alloc::vec![f64_bits(as_f64(arg_u64(args, 0)?) - as_f64(arg_u64(args, 1)?))],
            F64Mul => alloc::vec![f64_bits(as_f64(arg_u64(args, 0)?) * as_f64(arg_u64(args, 1)?))],
            F64Div => alloc::vec![f64_bits(as_f64(arg_u64(args, 0)?) / as_f64(arg_u64(args, 1)?))],
            ...
            F64Copysign => alloc::vec![f64_bits(as_f64(arg_u64(args, 0)?).copysign(as_f64(arg_u64(args, 1)?)))],
            F32Eq => alloc::vec![u64::from((as_f32(arg_u32(args, 0)?) == as_f32(arg_u32(args, 1)?)) as u32)],
            F32Ne => alloc::vec![u64::from((as_f32(arg_u32(args, 0)?) != as_f32(arg_u32(args, 1)?)) as u32)],
            F32Lt => alloc::vec![u64::from((as_f32(arg_u32(args, 0)?) < as_f32(arg_u32(args, 1)?)) as u32)],
            F32Gt => alloc::vec![u64::from((as_f32(arg_u32(args, 0)?) > as_f32(arg_u32(args, 1)?)) as u32)],
            F32Le => alloc::vec![u64::from((as_f32(arg_u32(args, 0)?) <= as_f32(arg_u32(args, 1)?)) as u32)],
            F32Ge => alloc::vec![u64::from((as_f32(arg_u32(args, 0)?) >= as_f32(arg_u32(args, 1)?)) as u32)],
            F64Eq => alloc::vec![u64::from((as_f64(arg_u64(args, 0)?) == as_f64(arg_u64(args, 1)?)) as u32)],
            F64Ne => alloc::vec![u64::from((as_f64(arg_u64(args, 0)?) != as_f64(arg_u64(args, 1)?)) as u32)],
            F64Lt => alloc::vec![u64::from((as_f64(arg_u64(args, 0)?) < as_f64(arg_u64(args, 1)?)) as u32)],
            F64Gt => alloc::vec![u64::from((as_f64(arg_u64(args, 0)?) > as_f64(arg_u64(args, 1)?)) as u32)],
            F64Le => alloc::vec![u64::from((as_f64(arg_u64(args, 0)?) <= as_f64(arg_u64(args, 1)?)) as u32)],
            F64Ge => alloc::vec![u64::from((as_f64(arg_u64(args, 0)?) >= as_f64(arg_u64(args, 1)?)) as u32)],
            ...
            Drop | Nop => Vec::new(),
            Unreachable => return Err(WasmError::invalid("native leaf unreachable executed".into())),
            ...
            MemoryGrow { mem_idx } => alloc::vec![self.memory_grow(*mem_idx, arg_u32(args, 0)? as usize)?],
            MemoryFill { imm0, .. } => {
                self.memory_fill(*imm0, arg_u32(args, 0)? as usize, arg_u32(args, 1)? as u8, arg_u32(args, 2)? as usize)?;
                Vec::new()
            }
            MemoryCopy { imm0, imm1 } => {
                self.memory_copy(*imm0, *imm1, arg_u32(args, 0)? as usize, arg_u32(args, 1)? as usize, arg_u32(args, 2)? as usize)?;
                Vec::new()
            }
            MemoryInit { imm0, imm1 } => {
                self.memory_init(*imm0, *imm1, arg_u32(args, 0)? as usize, arg_u32(args, 1)? as usize, arg_u32(args, 2)? as usize)?;
                Vec::new()
            }
            DataDrop { data_idx } => {
            ...
            I32Load { offset, memidx } => alloc::vec![u64::from(self.load_i32(*memidx, arg_u32(args, 0)?, *offset)? as u32)],
            I64Load { offset, memidx } => alloc::vec![self.load_i64(*memidx, arg_u32(args, 0)?, *offset)? as u64],
            F32Load { offset, memidx } => alloc::vec![u64::from(self.load_u32(*memidx, arg_u32(args, 0)?, *offset)?)],
            F64Load { offset, memidx } => alloc::vec![self.load_u64(*memidx, arg_u32(args, 0)?, *offset)?],
            I32Load8S { offset, memidx } => alloc::vec![u64::from(self.load_i8(*memidx, arg_u32(args, 0)?, *offset)? as i32 as u32)],
            I32Load8U { offset, memidx } => alloc::vec![u64::from(self.load_u8(*memidx, arg_u32(args, 0)?, *offset)? as u32)],
            I32Load16S { offset, memidx } => alloc::vec![u64::from(self.load_i16(*memidx, arg_u32(args, 0)?, *offset)? as i32 as u32)],
            I32Load16U { offset, memidx } => alloc::vec![u64::from(self.load_u16(*memidx, arg_u32(args, 0)?, *offset)? as u32)],
            I64Load8S { offset, memidx } => alloc::vec![(self.load_i8(*memidx, arg_u32(args, 0)?, *offset)? as i64) as u64],
            I64Load8U { offset, memidx } => alloc::vec![self.load_u8(*memidx, arg_u32(args, 0)?, *offset)? as u64],
            I64Load16S { offset, memidx } => alloc::vec![(self.load_i16(*memidx, arg_u32(args, 0)?, *offset)? as i64) as u64],
            I64Load16U { offset, memidx } => alloc::vec![self.load_u16(*memidx, arg_u32(args, 0)?, *offset)? as u64],
            I64Load32S { offset, memidx } => alloc::vec![(self.load_i32(*memidx, arg_u32(args, 0)?, *offset)? as i64) as u64],
            I64Load32U { offset, memidx } => alloc::vec![self.load_u32(*memidx, arg_u32(args, 0)?, *offset)? as u64],
            ...
            I32Store16 { offset, memidx } => {
                self.store_u16(*memidx, arg_u32(args, 0)?, *offset, arg_u32(args, 1)? as u16)?;
                Vec::new()
            }
            ...
            I64Store16 { offset, memidx } => {
                self.store_u16(*memidx, arg_u32(args, 0)?, *offset, arg_u64(args, 1)? as u16)?;
                Vec::new()
            }
            I64Store32 { offset, memidx } => {
                self.store_u32(*memidx, arg_u32(args, 0)?, *offset, arg_u64(args, 1)? as u32)?;
                Vec::new()
            }
            TableGet { table_idx } => alloc::vec![self.table_get(*table_idx, arg_u32(args, 0)? as usize)?],
            TableSet { table_idx } => {
            ...
            TableGrow { table_idx } => alloc::vec![self.table_grow(*table_idx, arg_u64(args, 0)?, arg_u32(args, 1)? as usize)?],
            TableFill { imm0, .. } => {
                self.table_fill(*imm0, arg_u32(args, 0)? as usize, arg_u64(args, 1)?, arg_u32(args, 2)? as usize)?;
                Vec::new()
            }
            TableCopy { imm0, imm1 } => {
                self.table_copy(*imm0, *imm1, arg_u32(args, 0)? as usize, arg_u32(args, 1)? as usize, arg_u32(args, 2)? as usize)?;
                Vec::new()
            }
            TableInit { imm0, imm1 } => {
                self.table_init(*imm0, *imm1, arg_u32(args, 0)? as usize, arg_u32(args, 1)? as usize, arg_u32(args, 2)? as usize)?;
                Vec::new()
            }
            ElemDrop { elem_idx } => {
    ...
    fn execute_call_external(
        ...
        match self.resolve_callee(func_idx)? {
            ResolvedCallee::External { func_type, callback } => {
                if func_type.params().len() != arg_values.len() || func_type.results().len() != results.len()
                {
    ...
    fn execute_call_local(
        ...
        match self.resolve_callee(callee)? {
            ResolvedCallee::Local {
                ...
                params_len,
                results_len,
            } => self.invoke_local(code, unsafe { &*program }, params_len, results_len, &arg_values, results),
    ...
    fn execute_call_indirect(
        ...
        let func_idx = {
            ...
            if handle.is_null() {
                return Err(WasmError::invalid("call_indirect: null function reference".into()));
            }
...

        {
            ...
            if expected.as_ref() != actual {
                return Err(WasmError::invalid("call_indirect: signature mismatch".into()));
            }
        }
    ...
    fn resolve_callee(&self, func_idx: u32) -> Result<ResolvedCallee, WasmError> {
        ...
        Ok(match func {
            ...
            FunctionInst::External { func_type, callback } => ResolvedCallee::External {
    ...
    fn store_u8(&mut self, mem_idx: u32, address: u32, offset: u32, value: u8) -> Result<(), WasmError> {
    ...
    fn table_set(&mut self, table_idx: u32, elem_index: usize, value: u64) -> Result<(), WasmError> {
        let store = self.store_mut()?;
...
mod tests {
    ...
    use crate::vm::{
        ...
        native::{
            ...
            ir::{NativeBlock, NativeBlockId, NativeInst, NativeInstKind, NativeProgram, NativeTerminator},
    ...
    fn executes_simple_add_program() {
        ...
        frame_mem[0] = 3;
        frame_mem[1] = 4;
        let mut ctx = NativeContext::new(core::ptr::null_mut(), unsafe {
            frame_mem.as_mut_ptr().add(frame_mem.len())
        }, core::ptr::null());
