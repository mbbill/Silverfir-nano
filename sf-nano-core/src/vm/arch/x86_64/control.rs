...
impl<'a> X86_64Backend<'a> {
    ...
    fn lower_return_sequence(&mut self) -> Result<(), WasmError> {
        ...
        match self.core.function.id.0 {
            205 => {
                enc::mov_rr_64(&mut self.core.text, C_ARG0, fp);
                enc::mov_rr_64(
                    &mut self.core.text,
                    C_ARG1,
                ...
                self.materialize_u64(
                    ...
                    super::helpers::x86_64_debug_dump_f205_return as usize as u64,
                );
                enc::call_reg(&mut self.core.text, self.gp_scratch.reg(0));
            }
            316 => {
                enc::mov_rr_64(&mut self.core.text, C_ARG0, fp);
                enc::mov_rr_64(
                ...
                self.materialize_u64(
                    self.gp_scratch.reg(0),
                    super::helpers::x86_64_debug_dump_f316_return as usize as u64,
                );
                enc::call_reg(&mut self.core.text, self.gp_scratch.reg(0));
            }
            418 => {
                ...
                self.materialize_u64(
                    self.gp_scratch.reg(0),
                    super::helpers::x86_64_debug_dump_f418_return as usize as u64,
                );
                enc::call_reg(&mut self.core.text, self.gp_scratch.reg(0));
            }
            717 => {
                ...
                self.materialize_u64(
                    self.gp_scratch.reg(0),
                    super::helpers::x86_64_debug_dump_f717_return as usize as u64,
                );
                enc::call_reg(&mut self.core.text, self.gp_scratch.reg(0));
            }
            720 => {
                ...
                self.materialize_u64(
                    self.gp_scratch.reg(0),
                    super::helpers::x86_64_debug_dump_f720_return as usize as u64,
                );
                enc::call_reg(&mut self.core.text, self.gp_scratch.reg(0));
            }
            721 => {
                ...
                self.materialize_u64(
                    self.gp_scratch.reg(0),
                    super::helpers::x86_64_debug_dump_f721_return as usize as u64,
                );
                enc::call_reg(&mut self.core.text, self.gp_scratch.reg(0));
            }
            _ => {}
    ...
    /// callee's frame base, emits `mov r11, imm64` + `call r11` where the
    /// imm64 is patched to the callee's internal entry at module-link
    ...
    fn lower_call_direct(
        ...
        enc::movabs_ri_64(&mut self.core.text, self.gp_scratch.reg(1), 0);
        ...
        // CALL r11 — pushes the return address (= this call site's
    ...
    pub(super) fn lower_call_external_term(
        ...
        let call_scratch = self.gp_scratch.reg(1);
        self.materialize_u64(
            call_scratch,
    ...
    fn lower_jump_table(
        ...
        match index {
            MachineValue::Imm64(value) => {
                self.materialize_u64(self.gp_scratch.reg(1), value as u32 as u64);
            }
            MachineValue::Reg(reg) => {
                let index_reg = self.map_gp_reg(reg)?;
                enc::mov_rr_32(&mut self.core.text, self.gp_scratch.reg(1), index_reg);
            }
            MachineValue::ReservedReg(reg) => {
                return Err(WasmError::internal(alloc::format!(
        ...
        enc::cmp_rr_32(&mut self.core.text, self.gp_scratch.reg(1), X86Reg::RAX);
        enc::cmovcc_rr_32(
            &mut self.core.text,
            Cc::A,
            self.gp_scratch.reg(1),
