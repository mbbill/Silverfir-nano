...
use crate::{
    ...
    vm::{
        machine::machine_ir::{
            ...
            MachineInst, MachineFunction, MachineReg, MachineTerminator,
    ...
    fn lower_source_move_dispatch(
        ...
        match src {
            ...
            ParallelSource::FpTemp(id, width) => {
                ...
                let dst_fp = self.map_fp_reg(dst.reg)? as u8;
                match width {
                    MachineFloatWidth::F32 => enc::movss_rr(&mut self.core.text, dst_fp, temp as u8),
                    MachineFloatWidth::F64 => enc::movsd_rr(&mut self.core.text, dst_fp, temp as u8),
                };
                self.core.set_fp_reg_width(dst.reg, width)?;
            }
        }
