...
pub(super) fn compile_inst(
    ...
    match &inst.kind {
        ...
        MachineInstKind::BitfieldExtractU { width, dst, src, lsb, bits } => {
            compile_bitfield_extract_u(fc, *width, *dst, *src, *lsb, *bits)?;
        }
        MachineInstKind::IntBinaryShifted { width, op, dst, lhs, rhs, shift, amount } => {
            compile_int_binary_shifted(fc, *width, *op, *dst, *lhs, *rhs, *shift, *amount)?;
        }
        MachineInstKind::TestBits { width, kind, dst, src, mask } => {
            compile_test_bits(fc, *width, *kind, *dst, *src, mask)?;
        }
    }
    Ok(())
}
...
fn compile_bitfield_extract_u(
    ...
    fc.text.emit_u32(enc::ubfx(dst_hw, src_hw, lsb as u32, bits as u32));
    Ok(())
}
