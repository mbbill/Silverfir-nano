...
//! - `local_facts.rs` — stable per-local facts used by planning
//! - `joint_plan/` — joint transient + local-cache planning
...
use self::{
    joint_plan::naive as naive_joint_plan,
...
use local_facts::collect_local_slot_facts;
...
pub(crate) fn prepare_function(
    ...
    let local_facts = collect_local_slot_facts(semantic, input.config.gp_unit_bytes, frame);
    let prepared = naive_joint_plan::plan_linear_semantics(
        semantic,
        frame,
        input.config,
        gp_dynamic_budget,
    ...
    if semantic.ops.is_empty() {
        return Ok(PreparedFunction {
            ...
            ssa: SsaProgram {
                entry: SsaTarget(0),
                blocks: Vec::new(),
                local_slot_types: local_facts.slot_types,
                local_slot_info: local_facts.slot_info,
    ...
    naive_joint_plan::plan_boundaries(
        &mut ssa,
        input.config.gp_unit_bytes,
        gp_dynamic_budget,
        fp_dynamic_budget,
