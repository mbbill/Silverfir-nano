...
fn prepares_explicit_cached_local_drop_before_call() {
    ...
    assert_eq!(prepared.ssa.local_slot_types.len(), 1);
    let has_cached_get = prepared
        .ssa
        .blocks
        .iter()
        .flat_map(|block| block.ops.iter())
        .any(|inst| matches!(inst.kind, SsaInstKind::LocalGetCache { .. }));
    ...
    let has_drop_before_call = prepared.ssa.blocks.iter().any(|block| {
        block
            .ops
            .iter()
            .position(|inst| matches!(inst.kind, SsaInstKind::Call(SsaCallOp::CallDirect { .. })))
