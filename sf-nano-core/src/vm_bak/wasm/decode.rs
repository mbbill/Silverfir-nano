...
impl<'a> IrLower<'a> {
    ...
    fn emit<K: Into<SemanticOpKind>>(&mut self, kind: K, window: u8) -> OpIndex {
        let kind = kind.into();
    ...
    fn emit_terminal<K: Into<SemanticOpKind>>(&mut self, kind: K, window: u8) -> OpIndex {
        let kind = kind.into();
        let idx = OpIndex::new(self.ops.len());
        let pre_height = self.pre_op_height(&kind);
        self.ops.push(SemanticOp {
    ...
    fn emit_with_target<K: Into<SemanticOpKind>>(&mut self, kind: K, window: u8) -> OpIndex {
        let kind = kind.into();
        let idx = OpIndex::new(self.ops.len());
        let pre_height = self.pre_op_height(&kind);
        self.ops.push(SemanticOp {
    ...
    // =========================================================================
    // Rotating-window helpers
    // =========================================================================

    /// Window rotation before the current op executes.
    #[inline]
    fn current_window(&self) -> u8 {
        self.stack.depth_window()
    }

    /// Window rotation for pop ops: pre-pop depth.
    #[inline]
    fn pre_pop_window(&self) -> u8 {
        self.stack.depth_window()
    }
    ...
    fn emit_cache_spill_if_needed(&mut self) {
        ...
        if self.stack.needs_spill_before_push() {
            ...
            let window = (((spill_depth % self.stack.config().tos_register_count) + 1)
                % self.stack.config().tos_register_count) as u8;
            self.stack.record_spill(1);
            self.emit(SemanticOpKind::CacheSpill { slot, count: 1 }, window);
        }
    }
    ...
    fn emit_cache_fill_for_operands(&mut self, operand_count: usize) {
        ...
        if spill_depth > min_spill_for_operands {
            ...
            let window = ((((spill_depth - 1) % self.stack.config().tos_register_count) + 1)
                % self.stack.config().tos_register_count) as u8;
            self.stack.record_fill(fill_count);
            self.emit(SemanticOpKind::CacheFill { slot, count: fill_count as u8 }, window);
        }
    }
    ...
    fn emit_cache_fill_if_needed(&mut self) {
        ...
        if fill_count > 0 {
            ...
            let window = ((((old_spill_depth - 1) % self.stack.config().tos_register_count) + 1)
                % self.stack.config().tos_register_count) as u8;
            self.emit(SemanticOpKind::CacheFill { slot, count: fill_count as u8 }, window);
        }
    }
    ...
    fn emit_cache_spill_all(&mut self) {
        ...
        if tos_count > 0 {
            ...
            let window = ((((height - 1) % self.stack.config().tos_register_count) + 1)
                % self.stack.config().tos_register_count) as u8;
            self.stack.record_spill(tos_count);
            self.emit(SemanticOpKind::CacheSpill { slot, count: tos_count as u8 }, window);
        }
    }
    ...
    fn emit_cache_spill_all_except_top(&mut self, keep_top: usize) {
        ...
        if to_spill > 0 {
            ...
            let window = ((((spill_depth + to_spill - 1) % self.stack.config().tos_register_count) + 1)
                % self.stack.config().tos_register_count) as u8;
            self.stack.record_spill(to_spill);
            self.emit(SemanticOpKind::CacheSpill { slot, count: to_spill as u8 }, window);
        }
    }
    ...
    fn emit_return(&mut self, arity: usize) {
        ...
        match arity {
            0 => {
                self.emit_terminal(SemanticOpKind::ReturnVoid, window);
            }
            1 => {
                self.emit_terminal(SemanticOpKind::ReturnOne, window);
            }
            _ => {
                self.emit_terminal(SemanticOpKind::Return { arity: arity as u16 }, window);
            }
        }
    }
}
