...
use crate::vm::plan::{CompileConfig, TosState};
...
impl StackTracker {
    ...
    /// Returns the current rotating-window head in the T-register cache.
    ///
    /// This matches the generated wrapper mapping in `build/fast_interp/gen_c_wrappers.rs`:
    /// - depth == 0 => D1
    /// - otherwise: ((depth - 1) % TOS_REGISTER_COUNT) + 1
    #[inline]
    pub fn depth_window(&self) -> u8 {
        self.tos.depth_window(self.height)
    }
