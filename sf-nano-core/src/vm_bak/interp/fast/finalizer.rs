...
use crate::vm::{wasm::StackTracker, lir::{IrOpKind, OpIndex, SlotRef}, abi::operand_encoding::encode_operands};
...
fn build_instructions(
    ...
    let instructions: Vec<Instruction> = ops
        .iter()
        .map(|op| {
            let (imm0, imm1, imm2) = encode_operands(&op.kind, op.frame_slot, 0, &fix_slot);
        ...
        if let Some(alt_idx) = op.alt_target {
            if op.has_target {
                unsafe {
                    ...
                    let (imm0, imm1, imm2) = encode_operands(&op.kind, op.frame_slot, target_ptr, &fix_slot);
