...
//!     → Box<[Instruction]>
//!
//! The backend-neutral compile frontend now lives in `vm::wasm`.
...
use crate::{
    ...
    vm::{
        ...
        wasm::{self, CompileContext, StackTracker},
        entities::ModuleInst,
...
#[cfg(feature = "fusion")]
fn resolve_fusion_backend(ir_ops: &[lir::IrOp]) -> Result<Vec<resolved::ResolvedInst>, &'static str> {
    Ok(super::fusion::resolve::resolve_fusion(ir_ops))
}

#[cfg(not(feature = "fusion"))]
fn resolve_fusion_backend(_ir_ops: &[lir::IrOp]) -> Result<Vec<resolved::ResolvedInst>, &'static str> {
...
pub fn build_for_function(
    ...
    let ir_ops = lir::lower_to_ir(code, &ctx, &mut stack, hot_local_plan)?;
    ...
    #[cfg(feature = "ir-dump")]
    super::dump::dump_resolved(&module.name, func_idx, &resolved, &ir_ops);

    // Finalize: Vec<ResolvedInst> → Box<[Instruction]>
    let code_box = super::finalizer::finalize(resolved, &mut stack);
...
mod tests {
    ...
    use crate::vm::{lir as ir, plan::FAST_COMPILE_CONFIG};
    ...
    fn depth_window_for(depth: u16) -> u8 {
        if depth == 0 {
            1
        } else {
    ...
    fn make_op(kind: lir::IrOpKind, _pre_height: u16, window: u8) -> lir::IrOp {
        lir::IrOp {
            kind,
            window,
            frame_slot: None,
    ...
    fn run_backend_with_hot(
        ...
        let mut code = super::super::finalizer::finalize(resolved, &mut stack);
    ...
    fn test_base_vs_jit_equivalent_with_br_terminator_no_fixup() {
        let mut br = make_op(
            ir::IrOpKind::Br { stack_drop: 1, arity: 0 },
            1,
            0,
        );
        br.has_target = true;
    ...
    fn test_base_vs_jit_equivalent_with_br_terminator_arity_no_fixup() {
        let mut br = make_op(
            ir::IrOpKind::Br { stack_drop: 0, arity: 1 },
            1,
            0,
        );
        br.has_target = true;
    ...
    fn test_base_vs_jit_equivalent_with_return_one_terminator() {
        let ops = vec![
            ...
            make_op(
                ir::IrOpKind::ReturnOne { frame_size: 0 },
                1,
                0,
            ),
        ];
