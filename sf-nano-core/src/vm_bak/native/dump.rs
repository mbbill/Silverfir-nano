...
pub fn flush_function(
    ...
    let Some(dir) = dump::subdir(module_name, func_idx, "native") else {
        return;
    };
    ...
    for (idx, entry) in entries.iter().enumerate() {
        ...
        let _ = dump::write_bytes(&blob_path, bytes);
    ...
    let _ = dump::write_text(&dir.join("index.tsv"), &index);
}

pub fn write_helper_metadata(
    module_name: &str,
    func_idx: u32,
    ir_index: usize,
    helper_name: &str,
    ...
    let path = dir.join(format!(
        "meta_ir{ir_index}_{}.txt",
        dump::sanitize_token(helper_name),
    ));
    let text = format!(
        "helper=0x{:x}\nnext_entry=0x{:x}\nnext_resume_slots=0x{:016x}\nbranch_entry=0x{:x}\nbranch_resume_slots=0x{:016x}\nframe_slot={}\ndata0=0x{:x}\ndata1=0x{:x}\ndata2=0x{:x}\n",
        metadata.helper as usize,
        metadata.next_entry,
        metadata.next_resume_slots,
        metadata.branch_entry,
        metadata.branch_resume_slots,
        metadata.frame_slot,
        metadata.data0,
        metadata.data1,
        metadata.data2,
...
fn blob_name(idx: usize, entry: &ArtifactRecord) -> String {
    format!(
        ...
        entry.ir_start,
        entry.ir_end,
        dump::sanitize_token(&entry.terminator)
    )
}
