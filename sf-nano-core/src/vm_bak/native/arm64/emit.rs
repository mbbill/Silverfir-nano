...
fn emit_trace_save_vm_regs(buf: &mut CodeBuffer) {
    buf.emit(enc::sub_imm_64(Reg::XZR, Reg::XZR, TRACE_SAVE_BYTES));
    buf.emit(enc::str_64(Reg::CTX, Reg::XZR, 0));
    buf.emit(enc::str_64(Reg::PC, Reg::XZR, 1));
    buf.emit(enc::str_64(Reg::FP, Reg::XZR, 2));
    buf.emit(enc::str_64(Reg::L0, Reg::XZR, 3));
    buf.emit(enc::str_64(Reg::L1, Reg::XZR, 4));
    buf.emit(enc::str_64(Reg::L2, Reg::XZR, 5));
    buf.emit(enc::str_64(Reg::T0, Reg::XZR, 6));
    buf.emit(enc::str_64(Reg::T1, Reg::XZR, 7));
    buf.emit(enc::str_64(Reg::T2, Reg::XZR, 8));
    buf.emit(enc::str_64(Reg::T3, Reg::XZR, 9));
    buf.emit(enc::str_64(Reg::TMP2, Reg::XZR, 10));
    buf.emit(enc::str_64(Reg::TMP3, Reg::XZR, 11));
    buf.emit(enc::str_64(Reg::TMP4, Reg::XZR, 12));
    buf.emit(enc::str_64(Reg::TMP5, Reg::XZR, 13));
}

#[cfg(feature = "function-trace")]
fn emit_trace_restore_vm_regs(buf: &mut CodeBuffer) {
    buf.emit(enc::ldr_64(Reg::CTX, Reg::XZR, 0));
    buf.emit(enc::ldr_64(Reg::PC, Reg::XZR, 1));
    buf.emit(enc::ldr_64(Reg::FP, Reg::XZR, 2));
    buf.emit(enc::ldr_64(Reg::L0, Reg::XZR, 3));
    buf.emit(enc::ldr_64(Reg::L1, Reg::XZR, 4));
    buf.emit(enc::ldr_64(Reg::L2, Reg::XZR, 5));
    buf.emit(enc::ldr_64(Reg::T0, Reg::XZR, 6));
    buf.emit(enc::ldr_64(Reg::T1, Reg::XZR, 7));
    buf.emit(enc::ldr_64(Reg::T2, Reg::XZR, 8));
    buf.emit(enc::ldr_64(Reg::T3, Reg::XZR, 9));
    buf.emit(enc::ldr_64(Reg::TMP2, Reg::XZR, 10));
    buf.emit(enc::ldr_64(Reg::TMP3, Reg::XZR, 11));
    buf.emit(enc::ldr_64(Reg::TMP4, Reg::XZR, 12));
    buf.emit(enc::ldr_64(Reg::TMP5, Reg::XZR, 13));
    buf.emit(enc::add_imm_64(Reg::XZR, Reg::XZR, TRACE_SAVE_BYTES));
}
...
fn emit_trace_guard_begin(buf: &mut CodeBuffer) -> usize {
    ...
    buf.emit(enc::ldr_64(Reg::TMP1, Reg::TMP0, 0));
    buf.emit(enc::cbz_64(Reg::TMP1, 0))
}
...
fn emit_trace_guard_end(buf: &mut CodeBuffer, skip_patch: usize) {
    ...
    buf.patch_u32(skip_patch, enc::cbz_64(Reg::TMP1, delta));
}
...
pub fn emit_direct_branch_literal(buf: &mut CodeBuffer, target_reg: Reg) -> usize {
    buf.emit(enc::ldr_lit_64(target_reg, 2));
    buf.emit(enc::br(target_reg));
    buf.emit_u64(0)
}
...
pub fn emit_helper_wrapper(
    ...
    buf.emit(enc::ldr_lit_64(Reg::PC, 4));
    buf.emit(enc::ldr_lit_64(Reg::TMP1, 5));
    buf.emit(enc::br(Reg::TMP1));
    buf.emit(enc::nop());
...
pub struct DirectCallInternalEmit {
    ...
    pub next_resume_slots_literal: usize,
}
...
pub fn emit_direct_br_table_entry(
    ...
    let start = buf.len();
    let table_ptr_load = buf.emit(enc::ldr_lit_64(Reg::TMP2, 0));
    materialize_u64(buf, Reg::TMP1, records.len().saturating_sub(1) as u64);

    // selected = min(idx, entry_count - 1)
    buf.emit(enc::mov_reg_64(Reg::TMP0, src_reg));
    buf.emit(enc::cmp_reg_64(Reg::TMP0, Reg::TMP1));
    buf.emit(enc::csel_64(Reg::TMP0, Reg::TMP0, Reg::TMP1, enc::Cond::LO));

    // record_ptr = table_base + selected * 16
    buf.emit(enc::lsl_imm_64(Reg::TMP3, Reg::TMP0, 4));
    buf.emit(enc::add_reg_64(Reg::TMP3, Reg::TMP2, Reg::TMP3));
    buf.emit(enc::mov_reg_64(Reg::TMP4, Reg::TMP3));

    buf.emit(enc::ldr_64(Reg::TMP5, Reg::TMP3, 1));

    // stack_drop in TMP0, arity in TMP5.
    buf.emit(enc::movz_64(Reg::A1, 0xffff, 0));
    buf.emit(enc::mov_reg_64(Reg::A0, Reg::TMP5));
    buf.emit(enc::lsr_imm_64(Reg::TMP0, Reg::TMP5, 16));
    buf.emit(enc::and_reg_64(Reg::TMP0, Reg::TMP0, Reg::A1));
    buf.emit(enc::and_reg_64(Reg::TMP5, Reg::A0, Reg::A1));

    materialize_u64(buf, Reg::A1, top_slot as u64);

    let skip_fixup_stack_drop = buf.emit(enc::cbz_64(Reg::TMP0, 0));
    let skip_fixup_arity = buf.emit(enc::cbz_64(Reg::TMP5, 0));

    // src_slot = (top_slot + 1) - arity
    buf.emit(enc::sub_reg_64(Reg::TMP3, Reg::A1, Reg::TMP5));
    buf.emit(enc::add_imm_64(Reg::TMP3, Reg::TMP3, 1));
    // dst_slot = src_slot - stack_drop
    buf.emit(enc::sub_reg_64(Reg::A0, Reg::TMP3, Reg::TMP0));
    buf.emit(enc::lsl_imm_64(Reg::TMP3, Reg::TMP3, 3));
    buf.emit(enc::lsl_imm_64(Reg::A0, Reg::A0, 3));
    buf.emit(enc::add_reg_64(Reg::TMP3, Reg::FP, Reg::TMP3));
    buf.emit(enc::add_reg_64(Reg::A0, Reg::FP, Reg::A0));
    let fixup_loop = buf.len();
    buf.emit(enc::ldr_64(Reg::A1, Reg::TMP3, 0));
    buf.emit(enc::str_64(Reg::A1, Reg::A0, 0));
    buf.emit(enc::add_imm_64(Reg::TMP3, Reg::TMP3, 8));
    buf.emit(enc::add_imm_64(Reg::A0, Reg::A0, 8));
    buf.emit(enc::subs_imm_64(Reg::TMP5, Reg::TMP5, 1));
    buf.emit(enc::b_cond(
        enc::Cond::NE,
    ...
    buf.patch_u32(
        ...
        enc::cbz_64(Reg::TMP0, ((after_fixup - skip_fixup_stack_drop) as i32) / 4),
    );
    buf.patch_u32(
        skip_fixup_arity,
        enc::cbz_64(Reg::TMP5, ((after_fixup - skip_fixup_arity) as i32) / 4),
    );

    buf.emit(enc::ldr_64(Reg::A0, Reg::TMP4, 0));
    buf.emit(enc::br(Reg::A0));
    ...
    buf.patch_u32(table_ptr_load, enc::ldr_lit_64(Reg::TMP2, table_ptr_delta));
...
pub fn emit_direct_call_internal_entry(
    ...
    buf.emit(enc::str_64(Reg::L0, Reg::FP, 0));
    buf.emit(enc::str_64(Reg::L1, Reg::FP, 1));
    buf.emit(enc::str_64(Reg::L2, Reg::FP, 2));

    materialize_u64(buf, Reg::TMP0, delta_slot as u64);
    buf.emit(enc::lsl_imm_64(Reg::TMP0, Reg::TMP0, 3));
    buf.emit(enc::add_reg_64(Reg::TMP3, Reg::FP, Reg::TMP0));

    let frame_size = params_count as u64 + locals_count as u64;
    materialize_u64(buf, Reg::TMP4, frame_size);
    buf.emit(enc::lsl_imm_64(Reg::TMP5, Reg::TMP4, 3));
    buf.emit(enc::add_reg_64(Reg::TMP5, Reg::TMP3, Reg::TMP5));
    buf.emit(enc::add_imm_64(Reg::TMP5, Reg::TMP5, 24));
    buf.emit(enc::ldr_64(Reg::TMP1, Reg::CTX, ctx_offset::STACK_END / 8));
    buf.emit(enc::cmp_reg_64(Reg::TMP5, Reg::TMP1));
    let stack_overflow_patch = buf.emit(enc::b_cond(enc::Cond::HI, 0));

    buf.emit(enc::ldr_64(Reg::TMP5, Reg::CTX, ctx_offset::CALL_DEPTH / 8));
    materialize_u64(buf, Reg::TMP1, 300);
    buf.emit(enc::cmp_reg_64(Reg::TMP5, Reg::TMP1));
    let call_depth_patch = buf.emit(enc::b_cond(enc::Cond::HS, 0));
    buf.emit(enc::add_imm_64(Reg::TMP5, Reg::TMP5, 1));
    buf.emit(enc::str_64(Reg::TMP5, Reg::CTX, ctx_offset::CALL_DEPTH / 8));

    if locals_count != 0 {
        materialize_u64(buf, Reg::TMP0, params_count as u64);
        buf.emit(enc::lsl_imm_64(Reg::TMP0, Reg::TMP0, 3));
        buf.emit(enc::add_reg_64(Reg::TMP5, Reg::TMP3, Reg::TMP0));
        let mut remaining = locals_count;
        while remaining >= 2 {
            buf.emit(enc::str_64(Reg::XZR, Reg::TMP5, 0));
            buf.emit(enc::str_64(Reg::XZR, Reg::TMP5, 1));
            buf.emit(enc::add_imm_64(Reg::TMP5, Reg::TMP5, 16));
            remaining -= 2;
        }
        if remaining != 0 {
            buf.emit(enc::str_64(Reg::XZR, Reg::TMP5, 0));
    ...
    buf.emit(enc::lsl_imm_64(Reg::TMP4, Reg::TMP4, 3));
    buf.emit(enc::add_reg_64(Reg::TMP5, Reg::TMP3, Reg::TMP4));
    let next_entry_load = buf.emit(enc::ldr_lit_64(Reg::TMP1, 0));
    let next_resume_slots_load = buf.emit(enc::ldr_lit_64(Reg::TMP0, 0));
    buf.emit(enc::str_64(Reg::TMP1, Reg::TMP5, 0));
    buf.emit(enc::str_64(Reg::FP, Reg::TMP5, 1));
    buf.emit(enc::str_64(Reg::TMP0, Reg::TMP5, 2));
    buf.emit(enc::mov_reg_64(Reg::FP, Reg::TMP3));
    buf.emit(enc::mov_reg_64(Reg::T0, Reg::XZR));
    buf.emit(enc::mov_reg_64(Reg::T1, Reg::XZR));
    buf.emit(enc::mov_reg_64(Reg::T2, Reg::XZR));
    buf.emit(enc::mov_reg_64(Reg::T3, Reg::XZR));
    #[cfg(feature = "function-trace")]
    let trace_entry_load = {
        let skip_patch = emit_trace_guard_begin(buf);
        let load = buf.emit(enc::ldr_lit_64(Reg::A1, 0));
        emit_trace_save_vm_regs(buf);
        buf.emit(enc::mov_reg_64(Reg::A0, Reg::CTX));
        materialize_u64(buf, Reg::TMP0, native_function_trace_enter_entry as usize as u64);
        buf.emit(enc::blr(Reg::TMP0));
        emit_trace_restore_vm_regs(buf);
        emit_trace_guard_end(buf, skip_patch);
        load
    };
    let callee_entry_load = buf.emit(enc::ldr_lit_64(Reg::TMP1, 0));
    buf.emit(enc::br(Reg::TMP1));

    let stack_overflow_label = buf.len();
    materialize_u64(buf, Reg::TMP0, STACK_OVERFLOW_MSG.as_ptr() as u64);
    buf.emit(enc::str_64(Reg::TMP0, Reg::CTX, ctx_offset::TRAP_MESSAGE / 8));
    buf.emit(enc::ldr_64(Reg::TMP0, Reg::CTX, ctx_offset::TERM_ENTRY / 8));
    buf.emit(enc::br(Reg::TMP0));
    ...
    buf.emit_u64(0);
    let next_resume_slots_literal = buf.len();
    buf.emit_u64(0);
    ...
    buf.patch_u32(
        ...
        enc::b_cond(enc::Cond::HI, stack_overflow_delta),
    );
    let call_depth_delta = ((call_stack_label - call_depth_patch) as i32) / 4;
    buf.patch_u32(
        call_depth_patch,
        enc::b_cond(enc::Cond::HS, call_depth_delta),
    );

    let next_entry_delta = ((next_entry_literal - next_entry_load) as i32) / 4;
    buf.patch_u32(next_entry_load, enc::ldr_lit_64(Reg::TMP1, next_entry_delta));
    let next_resume_slots_delta = ((next_resume_slots_literal - next_resume_slots_load) as i32) / 4;
    buf.patch_u32(
        next_resume_slots_load,
        enc::ldr_lit_64(Reg::TMP0, next_resume_slots_delta),
    );
    let callee_entry_delta = ((callee_entry_literal - callee_entry_load) as i32) / 4;
    buf.patch_u32(
        callee_entry_load,
        enc::ldr_lit_64(Reg::TMP1, callee_entry_delta),
    );
    #[cfg(feature = "function-trace")]
    {
        ...
        buf.patch_u32(
            trace_entry_load,
            enc::ldr_lit_64(Reg::A1, trace_entry_delta),
        );
    }
...
pub fn emit_function_trace_exit(buf: &mut CodeBuffer, arity: u16) {
    ...
    emit_trace_save_vm_regs(buf);
    buf.emit(enc::mov_reg_64(Reg::A0, Reg::CTX));
    buf.emit(enc::mov_reg_64(Reg::A1, Reg::FP));
...
pub fn emit_resume_entry(
    ...
    target_resume_slots: u64,
) -> (usize, usize) {
    let start = buf.len();
    materialize_u64(buf, Reg::A0, target_entry as usize as u64);
    if target_resume_slots == crate::vm::native::entry::EMPTY_RESUME_SLOTS {
        buf.emit(enc::mov_reg_64(Reg::T0, Reg::XZR));
        buf.emit(enc::mov_reg_64(Reg::T1, Reg::XZR));
        buf.emit(enc::mov_reg_64(Reg::T2, Reg::XZR));
        buf.emit(enc::mov_reg_64(Reg::T3, Reg::XZR));
    } else {
        materialize_u64(buf, Reg::A1, target_resume_slots);
        materialize_u64(
            buf,
            Reg::TMP1,
            native_helper_reload_tos_from_x1 as usize as u64,
        );
        buf.emit(enc::blr(Reg::TMP1));
    }
    buf.emit(enc::br(Reg::A0));
    (start, buf.len() - start)
}
...
/// branch target, optionally reloads the saved rotating-window contract, and
...
pub fn emit_meta_branch_wrapper(
    ...
    if reload_tos {
        buf.emit(enc::ldr_64(
            Reg::A1,
            Reg::PC,
            bridge::meta_offset::BRANCH_RESUME_SLOTS / 8,
        ));
        materialize_u64(
            buf,
            Reg::TMP1,
...
/// Packed saved window-contract state offset within helper metadata.
...
mod tests {
    ...
    fn test_dispatch_nonlinear_encoding() {
        ...
        unsafe {
            // ldr x3, [x21, #8]
            assert_eq!(*base.add(0), enc::ldr_64(Reg::TMP1, Reg::PC, 1));
            // ldr x2, [x3, #0]
            assert_eq!(*base.add(1), enc::ldr_64(Reg::TMP0, Reg::TMP1, 0));
            // ldr x1, [x3, #0x20]
            assert_eq!(*base.add(2), enc::ldr_64(Reg::NH, Reg::TMP1, INST_SIZE / 8));
            // mov x21, x3
            assert_eq!(*base.add(3), enc::mov_reg_64(Reg::PC, Reg::TMP1));
            // br x2
            assert_eq!(*base.add(4), enc::br(Reg::TMP0));
        }
    }
