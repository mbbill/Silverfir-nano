...
//!
//! `NativeEmitter` tracks the rotating T-window offset and emits ARM64
//! instructions for each supported Wasm opcode.
...
/// Compute depth window from stack height.
pub fn current_variant_from_window(window: u8) -> u8 {
    crate::vm::lir::current_variant_from_window(window)
}

#[inline]
fn window_add(window: u8, count: usize) -> u8 {
    ((window as usize + count) % 4) as u8
}

#[inline]
fn window_sub(window: u8, count: usize) -> u8 {
    ((window as usize + 4 - (count % 4)) % 4) as u8
}

/// Get TOS register at position `pos` (1=top) for the current rotating window.
pub fn tos_reg(window: u8, pos: u8) -> Reg {
    TOS_REGS[(window as usize + 4 - pos as usize) % 4]
}
...
impl<'a> NativeEmitter<'a> {
    pub fn new(buf: &'a mut CodeBuffer, initial_window: u8) -> Self {
    ...
    pub fn height(&self) -> usize { self.window as usize }

    fn dv(&self) -> u8 { current_variant_from_window(self.window) }
    ...
    fn pop2_push_materialized(&mut self) {
        ...
        self.slots[2] = self.slots[3];
        self.slots[3] = SlotLoc::canonical();
        self.window = window_sub(self.window, 1);
    }
    ...
    fn pop2(&mut self) {
        ...
        self.slots[3] = SlotLoc::canonical();
        self.window = window_sub(self.window, 2);
    }
    ...
    fn push_materialized_slot(&mut self) {
        self.window = window_add(self.window, 1);
    ...
    fn logical_pos_for_canonical_reg(&self, reg: Reg) -> Option<u8> {
        (1..=self.slots.len() as u8).find(|&pos| tos_reg(self.window, pos) == reg)
    }
    ...
    fn emit_reload_tos_from_packed(&mut self, packed_reg: Reg) {
        if packed_reg != Reg::TMP2 {
            self.buf.emit(enc::mov_reg_64(Reg::TMP2, packed_reg));
        }
        self.buf.emit(enc::mov_reg_64(Reg::T0, Reg::XZR));
        self.buf.emit(enc::mov_reg_64(Reg::T1, Reg::XZR));
        self.buf.emit(enc::mov_reg_64(Reg::T2, Reg::XZR));
        self.buf.emit(enc::mov_reg_64(Reg::T3, Reg::XZR));
        self.buf.emit(enc::movz_64(Reg::TMP3, 0xffff, 0));
        self.buf.emit(enc::movz_64(Reg::TMP5, 16, 0));

        for (i, dst) in [Reg::T0, Reg::T1, Reg::T2, Reg::T3].into_iter().enumerate() {
            self.buf.emit(enc::and_reg_64(Reg::TMP4, Reg::TMP2, Reg::TMP3));
            self.buf.emit(enc::cmp_reg_64(Reg::TMP4, Reg::TMP3));
            let skip = self.buf.emit(enc::b_cond(Cond::EQ, 0));
            self.buf.emit(enc::ldr_64_reg_scaled(dst, Reg::FP, Reg::TMP4));
            let delta = (self.buf.len() - skip) as i32 / 4;
            self.buf.patch_u32(skip, enc::b_cond(Cond::EQ, delta));
            if i != 3 {
                self.buf.emit(enc::lsrv_64(Reg::TMP2, Reg::TMP2, Reg::TMP5));
            }
        }
    }

    fn emit_reload_tos_from_target(&mut self, target_reg: Reg) {
        self.buf.emit(enc::ldr_64(
            Reg::TMP2,
            target_reg,
    ...
    fn materialize_aliases_from(&mut self, start_pos: usize) {
        for pos in start_pos..=self.slots.len() {
    ...
    fn materialize_slot_to_canonical(&mut self, pos: u8) {
        ...
        match self.slots[idx].gpr {
            ...
            Some(GprLoc::Alias(src)) => {
                if src != dst {
                    self.buf.emit(enc::mov_reg_64(dst, src));
                }
    ...
    fn emit_fp_to_gpr(&mut self, dst: Reg, fp: FpLoc) {
        match fp.width {
            FpWidth::F32 => self.buf.emit(enc::fmov_fp32_to_gpr(dst, fp.reg as u32)),
            FpWidth::F64 => self.buf.emit(enc::fmov_fp64_to_gpr(dst, fp.reg as u32)),
        };
    }
    ...
    fn ensure_tos_fp(&mut self, pos: u8, width: FpWidth) -> u8 {
        ...
        match width {
            FpWidth::F32 => self.buf.emit(enc::fmov_gpr_to_fp32(reg as u32, src)),
            FpWidth::F64 => self.buf.emit(enc::fmov_gpr_to_fp64(reg as u32, src)),
        };
    ...
    fn materialize_gpr_alias_reg(&mut self, reg: Reg) {
        for pos in 1..=self.slots.len() {
            ...
            if self.slots[idx].gpr == Some(GprLoc::Alias(reg)) {
                ...
                if dst != reg {
                    self.buf.emit(enc::mov_reg_64(dst, reg));
                }
                self.slots[idx].gpr = Some(GprLoc::Canonical);
            }
        }
    ...
    fn emit_branch_fixup_inline(
        ...
        let src_slot = top_slot + 1 - arity;
        let dst_slot = src_slot - stack_drop as u16;

        materialize_u64(self.buf, Reg::TMP3, (src_slot as u64) * 8);
        self.buf.emit(enc::add_reg_64(Reg::TMP3, Reg::FP, Reg::TMP3));
        materialize_u64(self.buf, Reg::A0, (dst_slot as u64) * 8);
        self.buf.emit(enc::add_reg_64(Reg::A0, Reg::FP, Reg::A0));
        materialize_u64(self.buf, Reg::TMP5, arity as u64);

        let loop_start = self.buf.len();
        self.buf.emit(enc::ldr_64(Reg::A1, Reg::TMP3, 0));
        self.buf.emit(enc::str_64(Reg::A1, Reg::A0, 0));
        self.buf.emit(enc::add_imm_64(Reg::TMP3, Reg::TMP3, 8));
        self.buf.emit(enc::add_imm_64(Reg::A0, Reg::A0, 8));
        self.buf.emit(enc::subs_imm_64(Reg::TMP5, Reg::TMP5, 1));
        self.buf.emit(enc::b_cond(
            Cond::NE,
    ...
    pub fn finish_br_general(
        ...
        self.materialize_aliases();
        self.emit_branch_fixup_inline(top_slot, stack_drop, arity);
        let alt_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        self.emit_trap_stub_if_needed();
        FinishInfo {
            start_offset: self.start_offset,
    ...
    pub fn finish_br_if_simple(mut self) -> FinishInfo {
        ...
        self.buf.emit(enc::cbz_32(cond, 5));
        let alt_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        let fallthrough_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
    ...
    pub fn finish_br_if_general(
        ...
        let branch_zero = self.buf.emit(enc::cbz_32(cond, 0));
        self.emit_branch_fixup_inline(top_slot, stack_drop, arity);
        let alt_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        let fallthrough_start = self.buf.len();
        self.buf.patch_u32(
            branch_zero,
            enc::cbz_32(cond, ((fallthrough_start - branch_zero) as i32) / 4),
        );
        let fallthrough_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
    ...
    pub fn finish_if(mut self) -> FinishInfo {
        ...
        self.buf.emit(enc::cbnz_32(cond, 5));
        let alt_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        let fallthrough_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
    ...
    pub fn finish_br_if_simple_from_flags(mut self, taken_cond: Cond, stack_pops: u8) -> FinishInfo {
        ...
        self.buf.emit(enc::b_cond(taken_cond, 5));
        let fallthrough_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        let alt_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        self.emit_trap_stub_if_needed();
        FinishInfo {
    ...
    pub fn finish_if_from_flags(mut self, linear_cond: Cond, stack_pops: u8) -> FinishInfo {
        ...
        self.buf.emit(enc::b_cond(linear_cond, 5));
        let alt_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        let fallthrough_literal = Some(emit::emit_direct_branch_literal(self.buf, Reg::TMP0));
        self.emit_trap_stub_if_needed();
        FinishInfo {
    ...
    fn emit_return_result_copy(&mut self, dst_slot: u32, stack_pos_from_top: u16, results: ReturnResults) {
        let src_slot = results.top_slot + 1 - stack_pos_from_top;
        self.buf.emit(enc::ldr_64(Reg::TMP2, Reg::FP, src_slot as u32));
        self.buf.emit(enc::str_64(Reg::TMP2, Reg::FP, dst_slot));
    }
    ...
    fn finish_return(
        ...
        self.buf.emit(enc::ldr_64(Reg::TMP1, Reg::FP, frame_size as u32));
        self.buf.emit(enc::ldr_64(Reg::TMP0, Reg::FP, frame_size as u32 + 1));
        self.buf.emit(enc::ldr_64(Reg::TMP5, Reg::FP, frame_size as u32 + 2));
        ...
        let term_patch = self.buf.emit(enc::cbz_64(Reg::TMP0, 0));
        ...
        self.buf.emit(enc::sub_imm_64(Reg::TMP2, Reg::TMP2, 1));
        self.buf.emit(enc::str_64(
            Reg::TMP2,
            Reg::CTX,
            emit::ctx_offset::CALL_DEPTH / 8,
        ));

        self.buf.emit(enc::mov_reg_64(Reg::FP, Reg::TMP0));
        self.buf.emit(enc::ldr_64(Reg::L0, Reg::FP, 0));
        self.buf.emit(enc::ldr_64(Reg::L1, Reg::FP, 1));
        self.buf.emit(enc::ldr_64(Reg::L2, Reg::FP, 2));
        let skip_reload = self.buf.emit(enc::cbz_64(Reg::TMP5, 0));
        self.emit_reload_tos_from_packed(Reg::TMP5);
        let after_reload = self.buf.len();
        self.buf.patch_u32(
            skip_reload,
            enc::cbz_64(Reg::TMP5, ((after_reload - skip_reload) as i32) / 4),
        );
        self.buf.emit(enc::br(Reg::TMP1));
        ...
        self.buf.emit(enc::br(Reg::TMP0));

        let delta = (term_offset - term_patch) as i32 / 4;
        self.buf
            .patch_u32(term_patch, enc::cbz_64(Reg::TMP0, delta));
    ...
    fn emit_trap_stub_if_needed(&mut self) {
        ...
        self.buf.emit(enc::str_64(Reg::TMP1, Reg::CTX,
            emit::ctx_offset::TRAP_MESSAGE / 8));

        // Branch to the runtime terminal entry.
        self.buf.emit(enc::ldr_64(Reg::TMP0, Reg::CTX,
            emit::ctx_offset::TERM_ENTRY / 8));
        self.buf.emit(enc::br(Reg::TMP0));
        ...
        for &patch_off in &self.trap_patches {
            ...
            self.buf.patch_u32(patch_off, enc::b_cond(Cond::HI, delta));
        }
    }
    ...
    fn patch_cbnz_32_to_here(&mut self, patch_off: usize, reg: Reg) {
        ...
        self.buf.patch_u32(patch_off, enc::cbnz_32(reg, delta));
    }

    fn patch_cbnz_64_to_here(&mut self, patch_off: usize, reg: Reg) {
        let delta = (self.buf.len() - patch_off) as i32 / 4;
        self.buf.patch_u32(patch_off, enc::cbnz_64(reg, delta));
    }

    fn patch_b_cond_to_here(&mut self, patch_off: usize, cond: Cond) {
        let delta = (self.buf.len() - patch_off) as i32 / 4;
        self.buf.patch_u32(patch_off, enc::b_cond(cond, delta));
    }
    ...
    pub fn i32_add(&mut self)   { self.emit_binop_32(enc::add_reg_32); }
    pub fn i32_sub(&mut self)   { self.emit_binop_32(enc::sub_reg_32); }
    pub fn i32_mul(&mut self)   { self.emit_binop_32(enc::mul_32); }
    pub fn i32_div_u(&mut self) {
        let lhs_loc = self.slot_int_loc(2);
        let lhs = self.resolve_tos_reg(2);
        let rhs = self.slot_gpr_source(1);
        let fast = self.buf.emit(enc::cbnz_32(rhs, 0));
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_32_to_here(fast, rhs);
        self.buf.emit(enc::udiv_32(lhs, lhs, rhs));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i32_div_s(&mut self) {
        ...
        let nonzero = self.buf.emit(enc::cbnz_32(rhs, 0));
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_32_to_here(nonzero, rhs);

        self.buf.emit(enc::movn_32(Reg::TMP0, 0, 0));
        self.buf.emit(enc::cmp_reg_32(rhs, Reg::TMP0));
        let rhs_not_minus_one = self.buf.emit(enc::b_cond(Cond::NE, 0));
        materialize_u32(self.buf, Reg::TMP0, 0x8000_0000);
        self.buf.emit(enc::cmp_reg_32(lhs, Reg::TMP0));
        let lhs_not_min = self.buf.emit(enc::b_cond(Cond::NE, 0));
        ...
        self.buf.emit(enc::sdiv_32(lhs, lhs, rhs));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i32_rem_u(&mut self) {
        ...
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_32_to_here(fast, rhs);
        self.buf.emit(enc::udiv_32(Reg::TMP0, lhs, rhs));
        self.buf.emit(enc::msub_32(lhs, Reg::TMP0, rhs, lhs));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i32_rem_s(&mut self) {
        ...
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_32_to_here(fast, rhs);
        self.buf.emit(enc::sdiv_32(Reg::TMP0, lhs, rhs));
        self.buf.emit(enc::msub_32(lhs, Reg::TMP0, rhs, lhs));
        self.pop2_push_loc(lhs_loc);
    }
    pub fn i32_and(&mut self)   { self.emit_binop_32(enc::and_reg_32); }
    pub fn i32_or(&mut self)    { self.emit_binop_32(enc::orr_reg_32); }
    pub fn i32_xor(&mut self)   { self.emit_binop_32(enc::eor_reg_32); }
    pub fn i32_shl(&mut self)   { self.emit_binop_32(enc::lslv_32); }
    pub fn i32_shr_u(&mut self) { self.emit_binop_32(enc::lsrv_32); }
    pub fn i32_shr_s(&mut self) { self.emit_binop_32(enc::asrv_32); }
    pub fn i32_rotr(&mut self)  { self.emit_binop_32(enc::rorv_32); }
    ...
    pub fn i32_rotl(&mut self) {
        ...
        let rhs = self.slot_gpr_source(1);
        // rotl(a, b) = rotr(a, -b)
        self.buf.emit(enc::neg_32(Reg::TMP0, rhs));
        self.buf.emit(enc::rorv_32(lhs, lhs, Reg::TMP0));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i64_add(&mut self)   { self.emit_binop_64(enc::add_reg_64); }
    pub fn i64_sub(&mut self)   { self.emit_binop_64(enc::sub_reg_64); }
    pub fn i64_mul(&mut self)   { self.emit_binop_64(enc::mul_64); }
    pub fn i64_div_u(&mut self) {
        let lhs_loc = self.slot_int_loc(2);
        let lhs = self.resolve_tos_reg(2);
        let rhs = self.slot_gpr_source(1);
        let fast = self.buf.emit(enc::cbnz_64(rhs, 0));
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_64_to_here(fast, rhs);
        self.buf.emit(enc::udiv_64(lhs, lhs, rhs));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i64_div_s(&mut self) {
        ...
        let nonzero = self.buf.emit(enc::cbnz_64(rhs, 0));
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_64_to_here(nonzero, rhs);

        materialize_u64(self.buf, Reg::TMP0, u64::MAX);
        self.buf.emit(enc::cmp_reg_64(rhs, Reg::TMP0));
        let rhs_not_minus_one = self.buf.emit(enc::b_cond(Cond::NE, 0));
        materialize_u64(self.buf, Reg::TMP0, 0x8000_0000_0000_0000);
        self.buf.emit(enc::cmp_reg_64(lhs, Reg::TMP0));
        let lhs_not_min = self.buf.emit(enc::b_cond(Cond::NE, 0));
        self.emit_inline_trap(INT_OVERFLOW_MSG);
        self.patch_b_cond_to_here(rhs_not_minus_one, Cond::NE);
        self.patch_b_cond_to_here(lhs_not_min, Cond::NE);

        self.buf.emit(enc::sdiv_64(lhs, lhs, rhs));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i64_rem_u(&mut self) {
        ...
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_64_to_here(fast, rhs);
        self.buf.emit(enc::udiv_64(Reg::TMP0, lhs, rhs));
        self.buf.emit(enc::msub_64(lhs, Reg::TMP0, rhs, lhs));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i64_rem_s(&mut self) {
        ...
        self.emit_inline_trap(INT_DIV_ZERO_MSG);
        self.patch_cbnz_64_to_here(fast, rhs);
        self.buf.emit(enc::sdiv_64(Reg::TMP0, lhs, rhs));
        self.buf.emit(enc::msub_64(lhs, Reg::TMP0, rhs, lhs));
        self.pop2_push_loc(lhs_loc);
    }
    pub fn i64_and(&mut self)   { self.emit_binop_64(enc::and_reg_64); }
    pub fn i64_or(&mut self)    { self.emit_binop_64(enc::orr_reg_64); }
    pub fn i64_xor(&mut self)   { self.emit_binop_64(enc::eor_reg_64); }
    pub fn i64_shl(&mut self)   { self.emit_binop_64(enc::lslv_64); }
    pub fn i64_shr_u(&mut self) { self.emit_binop_64(enc::lsrv_64); }
    pub fn i64_shr_s(&mut self) { self.emit_binop_64(enc::asrv_64); }
    pub fn i64_rotr(&mut self)  { self.emit_binop_64(enc::rorv_64); }
    ...
    pub fn i64_rotl(&mut self) {
        ...
        let rhs = self.slot_gpr_source(1);
        // rotl(a, b) = rotr(a, -b)
        self.buf.emit(enc::neg_64(Reg::TMP0, rhs));
        self.buf.emit(enc::rorv_64(lhs, lhs, Reg::TMP0));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    fn emit_cmp_32(&mut self, cond: Cond) {
        ...
        let lhs = self.resolve_tos_reg(2);
        let rhs = self.slot_gpr_source(1);
        self.buf.emit(enc::subs_reg_32(Reg::XZR, lhs, rhs));
        self.buf.emit(enc::cset_32(lhs, cond));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    fn emit_cmp_64(&mut self, cond: Cond) {
        ...
        let lhs = self.resolve_tos_reg(2);
        let rhs = self.slot_gpr_source(1);
        self.buf.emit(enc::subs_reg_64(Reg::XZR, lhs, rhs));
        self.buf.emit(enc::cset_64(lhs, cond));
        self.pop2_push_loc(lhs_loc);
    }
    ...
    pub fn i32_eqz(&mut self) {
        ...
        self.buf.emit(enc::cmp_imm_32(src, 0));
        self.buf.emit(enc::cset_32(dst, Cond::EQ));
        self.slots[0] = SlotLoc::canonical();
        // height unchanged
    }
    ...
    pub fn i32_clz(&mut self) {
        ...
        self.buf.emit(enc::clz_32(src, src));
    ...
    pub fn i32_ctz(&mut self) {
        let result_loc = self.slot_int_loc(1);
        let src = self.resolve_tos_reg(1);
        self.buf.emit(enc::rbit_32(src, src));
        self.buf.emit(enc::clz_32(src, src));
        self.slots[0] = result_loc;
    }
    ...
    pub fn i64_eqz(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = tos_reg(self.dv(), 1);
        self.buf.emit(enc::cmp_imm_64(src, 0));
        self.buf.emit(enc::cset_64(dst, Cond::EQ));
        self.slots[0] = SlotLoc::canonical();
    }
    ...
    pub fn i64_clz(&mut self) {
        let result_loc = self.slot_int_loc(1);
        let src = self.resolve_tos_reg(1);
        self.buf.emit(enc::clz_64(src, src));
        self.slots[0] = result_loc;
    }
    ...
    pub fn i64_ctz(&mut self) {
        let result_loc = self.slot_int_loc(1);
        let src = self.resolve_tos_reg(1);
        self.buf.emit(enc::rbit_64(src, src));
        self.buf.emit(enc::clz_64(src, src));
        self.slots[0] = result_loc;
    }
    ...
    pub fn local_get(&mut self, idx: u16) {
        ...
        self.buf.emit(enc::ldr_64(dst, Reg::FP, scaled));
    }
    ...
    pub fn local_set(&mut self, idx: u16) {
        ...
        let alias_loc = if let Some(fp) = self.slots[0].fp {
            match fp.width {
                FpWidth::F64 => {
                    self.buf.emit(enc::str_f64(fp.reg as u32, Reg::FP, scaled));
                    SlotLoc::fp_only(fp)
                }
                FpWidth::F32 => {
                    self.copy_slot_to_gpr(1, frame_alias_gpr);
                    self.buf.emit(enc::str_64(frame_alias_gpr, Reg::FP, scaled));
    ...
    fn init_one_hot_local(&mut self, slot: u16, target: u16, reg: Reg) {
        if target == slot {
            self.buf.emit(enc::ldr_64(reg, Reg::FP, target as u32));
            return;
        }

        self.buf.emit(enc::ldr_64(Reg::TMP0, Reg::FP, slot as u32));
        self.buf.emit(enc::ldr_64(reg, Reg::FP, target as u32));
        self.buf.emit(enc::str_64(Reg::TMP0, Reg::FP, target as u32));
        self.buf.emit(enc::str_64(reg, Reg::FP, slot as u32));
    }
    ...
    ///
    /// The `window` (from the IrOp) determines which physical registers to store.
    /// Height is unchanged (spill/fill have (0,0) stack effect in IR).
    pub fn spill(&mut self, slot: u16, count: u8, window: u8) {
        for i in 0..count as u32 {
            let reg = tos_reg(window, (i + 1) as u8);
            let pos = self
            ...
            self.buf.emit(enc::str_64(Reg::TMP0, Reg::FP, slot as u32 - i));
        }
    }

    /// Fill `count` TOS registers from memory at fp[slot..slot-count+1].
    ///
    /// The `window` (from the IrOp) determines which physical registers to load.
    /// Height is unchanged (spill/fill have (0,0) stack effect in IR).
    pub fn fill(&mut self, slot: u16, count: u8, window: u8) {
        for i in 0..count as u32 {
            let reg = tos_reg(window, (i + 1) as u8);
            self.buf.emit(enc::ldr_64(reg, Reg::FP, slot as u32 - i));
            let pos = self
                .logical_pos_for_canonical_reg(reg)
    ...
    fn emit_mem_bounds_check(&mut self, addr_reg: Reg, offset: u32, byte_count: u32) {
        ...
        self.buf.emit(enc::mov_reg_32(Reg::TMP0, addr_reg));

        // Add wasm memarg offset
        if offset > 0 {
            if offset <= 4095 {
                self.buf.emit(enc::add_imm_64(Reg::TMP0, Reg::TMP0, offset));
            } else {
                materialize_u32(self.buf, Reg::TMP1, offset);
                self.buf.emit(enc::add_reg_64(Reg::TMP0, Reg::TMP0, Reg::TMP1));
            }
        }

        // ea_end = ea + byte_count
        self.buf.emit(enc::add_imm_64(Reg::TMP1, Reg::TMP0, byte_count));

        self.ensure_mem0_size();

        // CMP ea_end, mem0_size (sets flags for unsigned comparison)
        self.buf.emit(enc::subs_reg_64(Reg::XZR, Reg::TMP1, MEM_CACHE_SIZE));

        // B.HI to trap stub (placeholder offset, patched by emit_trap_stub_if_needed)
        let patch_off = self.buf.emit(enc::b_cond(Cond::HI, 0));
    ...
    pub fn i32_load(&mut self, offset: u32) {
        ...
        let dst = tos_reg(self.dv(), 1);
        self.buf.emit(enc::ldr_32_reg(dst, MEM_CACHE_BASE, Reg::TMP0));
        self.slots[0] = SlotLoc::canonical();
    }
    ...
    pub fn f32_load(&mut self, offset: u32) {
        ...
        self.buf.emit(enc::ldr_f32_reg(dst as u32, MEM_CACHE_BASE, Reg::TMP0));
    ...
    pub fn i32_load8_u(&mut self, offset: u32) {
        ...
        self.buf.emit(enc::ldrb_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
    }
    ...
    pub fn i32_load8_s(&mut self, offset: u32) {
        ...
        self.emit_mem_bounds_check(reg, offset, 1);
        self.buf.emit(enc::ldrb_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
        self.buf.emit(enc::sxtb_32(reg, reg));
    }

    pub fn i32_load16_u(&mut self, offset: u32) {
        let reg = tos_reg(self.dv(), 1);
        self.emit_mem_bounds_check(reg, offset, 2);
        self.buf.emit(enc::ldrh_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
    }
    ...
    pub fn i32_load16_s(&mut self, offset: u32) {
        ...
        self.emit_mem_bounds_check(reg, offset, 2);
        self.buf.emit(enc::ldrh_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
        self.buf.emit(enc::sxth_32(reg, reg));
    }
    ...
    pub fn i64_load(&mut self, offset: u32) {
        ...
        let dst = tos_reg(self.dv(), 1);
        self.buf.emit(enc::ldr_64_reg(dst, MEM_CACHE_BASE, Reg::TMP0));
        self.slots[0] = SlotLoc::canonical();
    }
    ...
    pub fn f64_load(&mut self, offset: u32) {
        ...
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::ldr_f64_reg(dst as u32, MEM_CACHE_BASE, Reg::TMP0));
    ...
    pub fn i64_load8_s(&mut self, offset: u32) {
        ...
        self.emit_mem_bounds_check(reg, offset, 1);
        self.buf.emit(enc::ldrb_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
        self.buf.emit(enc::sxtb_64(reg, reg));
    }
    ...
    pub fn i64_load16_s(&mut self, offset: u32) {
        ...
        self.emit_mem_bounds_check(reg, offset, 2);
        self.buf.emit(enc::ldrh_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
        self.buf.emit(enc::sxth_64(reg, reg));
    }

    pub fn i64_load32_u(&mut self, offset: u32) {
        let reg = tos_reg(self.dv(), 1);
        self.emit_mem_bounds_check(reg, offset, 4);
        self.buf.emit(enc::ldr_32_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
    }
    ...
    pub fn i64_load32_s(&mut self, offset: u32) {
        ...
        self.emit_mem_bounds_check(reg, offset, 4);
        self.buf.emit(enc::ldr_32_reg(reg, MEM_CACHE_BASE, Reg::TMP0));
        self.buf.emit(enc::sxtw(reg, reg));
    }
    ...
    pub fn i32_store(&mut self, offset: u32) {
        ...
        self.emit_mem_bounds_check(addr_reg, offset, 4);
        self.buf.emit(enc::str_32_reg(val_reg, MEM_CACHE_BASE, Reg::TMP0));
        self.pop2();
    }
    ...
    pub fn f32_store(&mut self, offset: u32) {
        ...
        if let Some(fp) = self.slots[0].fp {
            ...
            self.buf.emit(enc::str_f32_reg(fp.reg as u32, MEM_CACHE_BASE, Reg::TMP0));
        } else {
            let val_reg = self.slot_gpr_source(1);
            self.buf.emit(enc::str_32_reg(val_reg, MEM_CACHE_BASE, Reg::TMP0));
        }
    ...
    pub fn i32_store8(&mut self, offset: u32) {
        ...
        self.buf.emit(enc::strb_reg(val_reg, MEM_CACHE_BASE, Reg::TMP0));
        self.pop2();
    }
    ...
    pub fn i32_store16(&mut self, offset: u32) {
        ...
        self.buf.emit(enc::strh_reg(val_reg, MEM_CACHE_BASE, Reg::TMP0));
        self.pop2();
    }
    ...
    pub fn i64_store(&mut self, offset: u32) {
        ...
        let addr_reg = self.slot_gpr_source(2);
        self.emit_mem_bounds_check(addr_reg, offset, 8);
        self.buf.emit(enc::str_64_reg(val_reg, MEM_CACHE_BASE, Reg::TMP0));
        self.pop2();
    }
    ...
    pub fn f64_store(&mut self, offset: u32) {
        ...
        if let Some(fp) = self.slots[0].fp {
            ...
            self.buf.emit(enc::str_f64_reg(fp.reg as u32, MEM_CACHE_BASE, Reg::TMP0));
        } else {
            let val_reg = self.slot_gpr_source(1);
            self.buf.emit(enc::str_64_reg(val_reg, MEM_CACHE_BASE, Reg::TMP0));
        }
    ...
    pub fn memory_size(&mut self, mem_idx: u32) {
        ...
        self.buf.emit(enc::lsr_imm_64(dst, dst, 16));
    }
    ...
    pub fn i32_wrap_i64(&mut self) {
        ...
        self.buf.emit(enc::mov_reg_32(src, src));
    }

    pub fn i64_extend_i32_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::sxtw(src, src));
    }
    ...
    pub fn i32_extend8_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::sxtb_32(src, src));
    }

    pub fn i32_extend16_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::sxth_32(src, src));
    }

    pub fn i64_extend8_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::sxtb_64(src, src));
    }

    pub fn i64_extend16_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::sxth_64(src, src));
    }
    ...
    pub fn select(&mut self) {
        ...
        let dst = tos_reg(d, 3);
        self.buf.emit(enc::cmp_imm_32(cond, 0));
        self.buf.emit(enc::csel_64(dst, val1, val2, Cond::NE));
    ...
    pub fn f64_add(&mut self) { self.emit_f64_binop(enc::fadd_64); }
    pub fn f64_sub(&mut self) { self.emit_f64_binop(enc::fsub_64); }
    pub fn f64_mul(&mut self) { self.emit_f64_binop(enc::fmul_64); }
    pub fn f64_div(&mut self) { self.emit_f64_binop(enc::fdiv_64); }
    pub fn f64_min(&mut self) { self.emit_f64_binop(enc::fmin_64); }
    pub fn f64_max(&mut self) { self.emit_f64_binop(enc::fmax_64); }
    ...
    pub fn f32_add(&mut self) { self.emit_f32_binop(enc::fadd_32); }
    pub fn f32_sub(&mut self) { self.emit_f32_binop(enc::fsub_32); }
    pub fn f32_mul(&mut self) { self.emit_f32_binop(enc::fmul_32); }
    pub fn f32_div(&mut self) { self.emit_f32_binop(enc::fdiv_32); }
    pub fn f32_min(&mut self) { self.emit_f32_binop(enc::fmin_32); }
    pub fn f32_max(&mut self) { self.emit_f32_binop(enc::fmax_32); }
    ...
    fn emit_f64_cmp(&mut self, cond: Cond) {
        let lhs = self.ensure_tos_f64(2);
        let rhs = self.ensure_tos_f64(1);
        let dst = tos_reg(window_sub(self.window, 1), 1);
        self.buf.emit(enc::fcmp_64(lhs, rhs));
        self.buf.emit(enc::cset_32(dst, cond));
    ...
    fn emit_f32_cmp(&mut self, cond: Cond) {
        ...
        let rhs = self.ensure_tos_f32(1);
        let dst = tos_reg(window_sub(self.window, 1), 1);
        self.buf.emit(enc::fcmp_32(lhs, rhs));
        self.buf.emit(enc::cset_32(dst, cond));
        self.pop2_push_materialized();
    }
    ...
    pub fn f64_abs(&mut self) { self.emit_f64_unop(enc::fabs_64); }
    pub fn f64_neg(&mut self) { self.emit_f64_unop(enc::fneg_64); }
    pub fn f64_sqrt(&mut self) { self.emit_f64_unop(enc::fsqrt_64); }
    pub fn f64_ceil(&mut self) { self.emit_f64_unop(enc::frintp_64); }
    pub fn f64_floor(&mut self) { self.emit_f64_unop(enc::frintm_64); }
    pub fn f64_trunc(&mut self) { self.emit_f64_unop(enc::frintz_64); }
    pub fn f64_nearest(&mut self) { self.emit_f64_unop(enc::frintn_64); }
    ...
    pub fn f32_abs(&mut self) { self.emit_f32_unop(enc::fabs_32); }
    pub fn f32_neg(&mut self) { self.emit_f32_unop(enc::fneg_32); }
    pub fn f32_sqrt(&mut self) { self.emit_f32_unop(enc::fsqrt_32); }
    pub fn f32_ceil(&mut self) { self.emit_f32_unop(enc::frintp_32); }
    pub fn f32_floor(&mut self) { self.emit_f32_unop(enc::frintm_32); }
    pub fn f32_trunc(&mut self) { self.emit_f32_unop(enc::frintz_32); }
    pub fn f32_nearest(&mut self) { self.emit_f32_unop(enc::frintn_32); }
    ...
    pub fn f64_promote_f32(&mut self) {
        let src = self.ensure_tos_f32(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::fcvt_f32_to_f64(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F64 });
    }
    ...
    pub fn f32_demote_f64(&mut self) {
        let src = self.ensure_tos_f64(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::fcvt_f64_to_f32(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F32 });
    }
    ...
    pub fn f32_convert_i32_s(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::scvtf_s_w(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F32 });
    }
    ...
    pub fn f32_convert_i32_u(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::ucvtf_s_w(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F32 });
    }
    ...
    pub fn f32_convert_i64_s(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::scvtf_s_x(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F32 });
    }
    ...
    pub fn f32_convert_i64_u(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::ucvtf_s_x(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F32 });
    }
    ...
    pub fn f64_convert_i32_s(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::scvtf_d_w(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F64 });
    }
    ...
    pub fn f64_convert_i32_u(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::ucvtf_d_w(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F64 });
    }
    ...
    pub fn f64_convert_i64_s(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::scvtf_d_x(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F64 });
    }
    ...
    pub fn f64_convert_i64_u(&mut self) {
        let src = self.slot_gpr_source(1);
        let dst = self.alloc_fp_result();
        self.buf.emit(enc::ucvtf_d_x(dst as u32, src));
        self.replace_top_with_fp(FpLoc { reg: dst, width: FpWidth::F64 });
    }
    ...
    pub fn i32_trunc_sat_f32_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp32(Self::FP0, src));
        self.buf.emit(enc::fcvtzs_w_s(src, Self::FP0));
    }

    pub fn i32_trunc_sat_f32_u(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp32(Self::FP0, src));
        self.buf.emit(enc::fcvtzu_w_s(src, Self::FP0));
    }

    pub fn i32_trunc_sat_f64_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp64(Self::FP0, src));
        self.buf.emit(enc::fcvtzs_w_d(src, Self::FP0));
    }

    pub fn i32_trunc_sat_f64_u(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp64(Self::FP0, src));
        self.buf.emit(enc::fcvtzu_w_d(src, Self::FP0));
    }

    pub fn i64_trunc_sat_f32_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp32(Self::FP0, src));
        self.buf.emit(enc::fcvtzs_x_s(src, Self::FP0));
    }

    pub fn i64_trunc_sat_f32_u(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp32(Self::FP0, src));
        self.buf.emit(enc::fcvtzu_x_s(src, Self::FP0));
    }

    pub fn i64_trunc_sat_f64_s(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp64(Self::FP0, src));
        self.buf.emit(enc::fcvtzs_x_d(src, Self::FP0));
    }

    pub fn i64_trunc_sat_f64_u(&mut self) {
        let src = tos_reg(self.dv(), 1);
        self.buf.emit(enc::fmov_gpr_to_fp64(Self::FP0, src));
        self.buf.emit(enc::fcvtzu_x_d(src, Self::FP0));
    }
...
fn materialize_u32(buf: &mut CodeBuffer, dst: Reg, value: u32) {
    ...
    if hi == 0 {
        buf.emit(enc::movz_32(dst, lo, 0));
    } else if lo == 0 {
        buf.emit(enc::movz_32(dst, hi, 16));
    } else {
        buf.emit(enc::movz_32(dst, lo, 0));
        buf.emit(enc::movk_32(dst, hi, 16));
    }
}

fn materialize_u64(buf: &mut CodeBuffer, dst: Reg, value: u64) {
    if value == 0 {
        buf.emit(enc::movz_64(dst, 0, 0));
        return;
    }
    ...
    for &(chunk, shift) in &chunks {
        if chunk != 0 || first {
            if first {
                buf.emit(enc::movz_64(dst, chunk, shift));
                first = false;
            } else {
                buf.emit(enc::movk_64(dst, chunk, shift));
            }
        }
    }
}
...
mod tests {
    ...
    fn run_jit_test(
        ...
        emitter.emit_raw(enc::str_64(result_reg, Reg::FP, 0));
    ...
    fn run_jit_test_local(
        ...
        emitter.materialize_aliases();
        emitter.emit_raw(enc::str_64(local_reg, Reg::FP, 0));
    ...
    // ==================== Unit tests: depth_window / tos_reg ====================

    #[test]
    fn test_depth_window() {
        assert_eq!(depth_window(0), 1);
        assert_eq!(depth_window(1), 1);
        assert_eq!(depth_window(2), 2);
        assert_eq!(depth_window(3), 3);
        assert_eq!(depth_window(4), 4);
        assert_eq!(depth_window(5), 1);
        assert_eq!(depth_window(6), 2);
    }
    ...
    fn test_init_locals_swaps_frame_and_fills_hot_registers() {
        ...
        let start;
        {
            ...
            e.init_locals(5, 3, 2);
            e.emit_raw(enc::str_64(Reg::L0, Reg::FP, 8));
            e.emit_raw(enc::str_64(Reg::L1, Reg::FP, 9));
            e.emit_raw(enc::str_64(Reg::L2, Reg::FP, 10));
            start = e.finish();
        }
    ...
    fn test_two_frame_aliases_avoid_reloads() {
        ...
        unsafe {
            ...
            for i in 0..(total_len / 4) {
                let inst = *base.add(i);
                if inst == enc::ldr_64(Reg::T0, Reg::FP, 1)
                    || inst == enc::ldr_64(Reg::T1, Reg::FP, 2)
                {
    ...
    fn test_br_if_simple_fallthrough() {
        ...
        let start;
        {
            ...
            e.emit_raw(enc::str_64(val_reg, Reg::FP, 0));
    ...
    fn test_br_if_simple_taken_multi_group() {
        ...
        let start2 = buf.len();
        {
            ...
            e.emit_raw(enc::str_64(top, Reg::FP, 1)); // fp[1] = 99
            e.finish();
        }
    ...
    fn test_br_if_simple_fallthrough_multi_group() {
        ...
        let start2 = buf.len();
        {
            ...
            e.i32_const(88);
            let top = tos_reg(e.dv(), 1);
            e.emit_raw(enc::str_64(top, Reg::FP, 1));
            e.finish();
        }
    ...
    fn test_br_taken_multi_group() {
        ...
        let start1;
        {
            ...
            e.i32_const(42);
            let top = tos_reg(e.dv(), 1);
            e.emit_raw(enc::str_64(top, Reg::FP, 0));
            start1 = e.finish_br();
        }
    ...
    fn test_mem_metadata_loads_are_hoisted_per_group() {
        ...
        unsafe {
            ...
            for i in 0..(total_len / 4) {
                let inst = *base.add(i);
                if inst == enc::ldr_64(MEM_CACHE_SIZE, Reg::CTX, emit::ctx_offset::MEM0_SIZE / 8) {
                    mem0_size_loads += 1;
                }
                if inst == enc::ldr_64(MEM_CACHE_BASE, Reg::CTX, emit::ctx_offset::MEM0_BASE / 8) {
                    mem0_base_loads += 1;
                }
            }
