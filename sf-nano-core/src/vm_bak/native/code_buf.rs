...
mod tests {
    ...
    fn test_emit_mov_ret() {
        ...
        let mut buf = CodeBuffer::new().expect("mmap failed");
        buf.begin_write();
        let offset = buf.emit(enc::movz_64(Reg::T3, 42, 0));
        buf.emit(enc::ret());
    ...
    fn test_emit_add_ret() {
        ...
        let mut buf = CodeBuffer::new().expect("mmap failed");
        buf.begin_write();
        let offset = buf.emit(enc::movz_64(Reg::T3, 10, 0));
        buf.emit(enc::add_imm_64(Reg::T3, Reg::T3, 32));
        buf.emit(enc::ret());
    ...
    fn test_multiple_emissions() {
        ...
        buf.begin_write();
        let off1 = buf.emit(enc::movz_64(Reg::T3, 10, 0));
        buf.emit(enc::ret());
        buf.finish_write(off1, 8);

        // Function 2: returns 20
        buf.begin_write();
        let off2 = buf.emit(enc::movz_64(Reg::T3, 20, 0));
        buf.emit(enc::ret());
