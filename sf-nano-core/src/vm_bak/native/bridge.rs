...
use crate::vm::lir::{IrOpKind, SlotRef, stack_effect};
...
use super::entry::{NativeEntry, EMPTY_RESUME_SLOTS};
...
pub struct HelperResult {
    ...
    pub next_resume_slots: u64,
}

impl HelperResult {
    #[inline]
    pub const fn new(next_entry: NativeEntry, next_resume_slots: u64) -> Self {
        Self {
            next_entry,
            next_resume_slots,
        }
...
impl HelperMetadata {
    ...
    pub fn new(
        helper: NativeHelper,
        next_entry: u64,
        next_resume_slots: u64,
        branch_entry: u64,
        branch_resume_slots: u64,
        frame_slot: u64,
        data0: u64,
        data1: u64,
        data2: u64,
...
pub mod meta_offset {
    ...
    pub const NEXT_RESUME_SLOTS: u32 = core::mem::offset_of!(HelperMetadata, next_resume_slots) as u32;
    pub const BRANCH_ENTRY: u32 = core::mem::offset_of!(HelperMetadata, branch_entry) as u32;
    pub const BRANCH_RESUME_SLOTS: u32 = core::mem::offset_of!(HelperMetadata, branch_resume_slots) as u32;
    pub const FRAME_SLOT: u32 = core::mem::offset_of!(HelperMetadata, frame_slot) as u32;
...
const _: [(); 16] = [(); meta_offset::NEXT_RESUME_SLOTS as usize];
const _: [(); 24] = [(); meta_offset::BRANCH_ENTRY as usize];
const _: [(); 32] = [(); meta_offset::BRANCH_RESUME_SLOTS as usize];
const _: [(); 40] = [(); meta_offset::FRAME_SLOT as usize];
...
fn resume_at(
    ...
    HelperResult::new(next_entry, next_resume_slots)
}
...
fn trap_with(ctx: *mut Context, fp: *mut u64, error: WasmError) -> HelperResult {
    ...
    resume_at(ctx, fp, term_entry(), EMPTY_RESUME_SLOTS)
}
...
fn debug_helper_site(store: &Store, func_idx: usize, meta_idx: usize) -> Option<alloc::string::String> {
    ...
    let plan = crate::vm::plan::CompilePlan::for_config(spec.code(), frame_size, config);
    let hot_local_plan = plan.hot_locals();
    let ctx = crate::vm::wasm::CompileContext::new(
        Some(store.module().types.as_slice()),
        store,
        store.module(),
        results_count,
    );
    let mut stack = crate::vm::wasm::StackTracker::new(
        plan.config(),
        params_count,
        locals_count,
        results_count,
    );
    let semantic_ops = crate::vm::wasm::decode::lower_to_semantic_ir(spec.code(), &ctx, &mut stack).ok()?;
    let group_plan = crate::vm::plan::plan_groups(
        &semantic_ops,
        &crate::vm::native::group_policy::NativeGroupPolicy::new(
            hot_local_plan,
            plan.config(),
            frame_size,
        ),
    );
    let lowered = crate::vm::lir::lower_semantic_to_program(
        semantic_ops,
        hot_local_plan,
        plan.config(),
        frame_size,
        ...
        .take(window_end - window_start)
    {
        ...
        resolved_window.push_str(&alloc::format!(
            "{}{}:{:?}/w{}",
            marker, idx, candidate.kind, candidate.window
        ));
    }
        ...
        let local_write = tracked_local.and_then(|original_local| {
            ...
            ir_ops
                ...
                .find(|(_, candidate)| {
                    matches!(
                        candidate.kind,
                        crate::vm::lir::IrOpKind::LocalSetFrame { idx }
                            | crate::vm::lir::IrOpKind::LocalTeeFrame { idx }
                            if idx == remapped
                    )
                })
        });
    ...
    Some(alloc::format!(
        "func={} type={} frame={} raw_hot={:?} eff_hot={:?} inverse={:?} cold_meta={} op_idx={} window={} kind={:?} window_trace=[{}]{}",
        func_idx,
        type_index,
        frame_size,
        hot_local_plan.raw(),
...
fn ensure_native_entry(
    ...
    if !spec.has_native_code() {
        ...
        {
            crate::vm::native::build::build_for_function(
                spec,
                Some(&module.types),
                store_ref,
                module,
...
fn enter_unified_callee(
    ...
    unsafe {
        ...
        *callee_fp.add(frame_size + 2) = return_resume_slots;
    }
    ...
    Ok(resume_at(ctx, callee_fp, entry, EMPTY_RESUME_SLOTS))
}
...
pub fn build_metadata<F: Fn(SlotRef) -> u16>(
    ...
    next_entry: NativeEntry,
    next_resume_slots: u64,
    branch_target: Option<(NativeEntry, u64)>,
    frame_slot: Option<SlotRef>,
    fix_slot: &F,
) -> HelperMetadata {
    let frame_slot = frame_slot.map_or(0, |slot| fix_slot(slot) as u64);
    let next_resume_slots = match helper_kind {
        ColdHelperKind::CallExternal | ColdHelperKind::CallInternal | ColdHelperKind::CallIndirect => {
            next_resume_slots
        }
        _ => EMPTY_RESUME_SLOTS,
    };
    let (branch_entry, branch_resume_slots) = match branch_target {
        Some((target_entry, target_resume_slots)) => {
            let slots = match helper_kind {
                ColdHelperKind::CallExternal
                | ColdHelperKind::CallInternal
                | ColdHelperKind::CallIndirect => target_resume_slots,
                _ => EMPTY_RESUME_SLOTS,
            };
            (target_entry as usize as u64, slots)
        }
        None => (0, EMPTY_RESUME_SLOTS),
    };
    match (helper_kind, kind) {
        (ColdHelperKind::CallExternal, IrOpKind::CallExternal { func_idx, delta, .. }) => {
            HelperMetadata::new(
                call_external_helper,
                next_entry as usize as u64,
                next_resume_slots,
                branch_entry,
                branch_resume_slots,
                frame_slot,
...
pub unsafe extern "C" fn call_indirect_helper(
    ...
    let delta = (*meta).data1 as usize;
    let index_slot = (*meta).frame_slot as usize;

    let store = store_mut(ctx);
    let elem_index = frame_read(fp, index_slot) as usize;
    ...
    if func_ref.is_null() {
        ...
        return trap_with(
            ...
            fp,
            WasmError::trap(alloc::format!(
                "uninitialized element: table={} index={} len={} elems=[{}] index_slot={} delta={} args=[{:#x},{:#x},{:#x}]{}{}",
                table_idx,
                elem_index,
                table.elements.len(),
                table_dump,
...
pub unsafe extern "C" fn global_get_helper(
    ...
    let slot = (*meta).frame_slot as usize;
    let store = store_mut(ctx);
...
pub unsafe extern "C" fn memory_copy_helper(
    ...
    let top = (*meta).frame_slot as usize;
...
pub unsafe extern "C" fn ref_null_helper(
    ...
    frame_write(fp, (*meta).frame_slot as usize, usize::MAX as u64);
    meta_next(_ctx, fp, meta)
}
...
pub unsafe extern "C" fn ref_func_helper(
    ...
    frame_write(fp, (*meta).frame_slot as usize, (*meta).data0);
    meta_next(_ctx, fp, meta)
}
