...
/* ---- self-timing benchmark mode ----
 *
 * The batch unit is one render of a small fixed image. A whole frame is the
 * unit because a partial frame would sample a different mix of pixels (rays
 * differ wildly in cost), which would make the reported rate depend on the
 * batch size. BENCH_RES is kept small so one unit stays cheap enough for a
 * slow engine to regulate against (see bench.h).
 */
#define BENCH_RES 64

static uint32_t *bench_fb;
static uint32_t  bench_checksum;

static void cray_batch(long n, void *ctx) {
	long k;
	int i;
	uint32_t sum = 0;
	(void)ctx;
	for(k=0; k<n; k++) {
		render(BENCH_RES, BENCH_RES, bench_fb, 1);
	}
	for(i=0; i<BENCH_RES * BENCH_RES; i++) sum = sum * 31u + bench_fb[i];
	bench_checksum = sum;
}
