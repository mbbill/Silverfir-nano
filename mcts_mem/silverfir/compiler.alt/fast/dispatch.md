...
- 2026-02-06 (33291768) rationale: the guard-check branch that selects the
  ...
  the generator still preloads but dispatches straight through with no guard (code).

- 2026-02-06 (33291768) rationale: the guard-check branch is omitted on Windows
  x86-64 because there the CPU's indirect-branch predictor already handles the
  common linear case well and the guard is net overhead; the preload-and-guard win
  is specific to in-order-ish/ARM dispatch where the dependent handler-pointer load
  and misprediction dominate (sourced).
...
- 2025-08-13 (2b4d0f10) pitfall: the op_* wrapper's tail call into the next handler
  ...
  be carried by a sentinel/exit handler instead (code).
...
- 2025-08-15 (7a4b152f) pitfall: the per-instruction successor pointers are absolute
  ...
  pointer dangling; patching now runs against the boxed allocation's base (code).
...
- 2025-08-16 (75b3519a) pitfall: the dynamic memory address index popped from the
  ...
  index+offset takes the trap edge (code).
...
- 2026-02-06 (04776e14) rationale: a specialized br_if_simple handler covers the
  ...
  misprediction-prone fast-backend opcode (code).
...
- 2025-10-01 (b7d4218c) rationale: the memarg memory index is encoded
  ...
  back to a store lookup by index only when the index is non-zero (code).
...
- 2025-10-05 (9b25df61) pitfall: the fast backend packs a memarg offset into a
  ...
  left for later when the offset field widens to u64 (code).
...
- 2025-12-10 (971a278c) rationale: the finalizer drops drop instructions from the
  ...
  than dispatched as a no-op handler (code).
...
- 2025-08-19 (5dcf7436) rationale: after the explicit fallthrough pointer was
  ...
  the instruction cache, even though only handler/alt/imm0/imm1 are live (code).
...
- 2025-08-14 (63d0d66b) rationale: linear-memory accesses bounds-check the
  ...
  plus one length check rather than re-reading the memory instance (code).
...
- 2025-08-17 (ce4fd170) replaced [[explicit-fallthrough-pointer]]: each
  ...
  per-instruction pointer field and its load on every non-branch op (code).
...
- 2025-12-10 (df2532aa) replaced [[out-of-band-br-table-blob]]: the separate
  ...
  blob-base pointer entirely (code).
