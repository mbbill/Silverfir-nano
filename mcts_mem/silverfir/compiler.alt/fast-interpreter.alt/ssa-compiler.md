...
- 2025-09-17 (aed2ff42) rationale: the SSA backend integrates only with
  interpreter-agnostic runtime components (Store, ModuleInst, MemInst,
  TableInst, Globals), cuts dispatches by selecting fused superinstructions from
  SSA expression trees (tile covering of depth <=2 chosen by Sethi-Ullman
  numbering, with store-root tiles computing their whole RHS in one dispatch),
  and cuts per-dispatch work via tiny direct-threaded handlers (code).

- 2025-09-17 (aed2ff42) statement: the SSA backend is specified as a clean-slate,
  stand-alone interpreter that deliberately inherits no layout or constraints
  from the fast (stack-window) backend, with a performance thesis to minimize the
  two interpreter costs at once — fewer dispatches and less per-dispatch work
  (sourced).
...
- 2026-06-14 statement: xir and fast share the same handler-table dispatch (xir
  ...
  [[fast-interpreter.fact/why-xir-died]] (sourced).
