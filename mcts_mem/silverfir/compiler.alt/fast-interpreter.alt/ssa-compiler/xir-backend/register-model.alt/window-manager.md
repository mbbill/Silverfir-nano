...
- 2025-10-24 (010b53d5) pitfall: the window is a write-back cache, so two
  ...
  duplicate occupancy loses the value (code).
...
- 2025-10-24 (010b53d5) pitfall: evicting a slot must always store the current
  ...
  needed), never to whether eviction writes back (code).
...
- 2025-10-24 (cc79153e) pitfall: for br_table the window must be flushed to the
  ...
  index — ordering is flush-then-load-index (code).
...
- 2025-10-25 (b2109ad2) pitfall: at a conditional branch the window is flushed,
  ...
  VReg rather than assume window contents survive the edge (code).
...
- 2025-10-30 (79b3eba8) pitfall: the dead-value store-skip used program-order
  ...
  while mid-block evictions keep the deadness heuristic (code).
...
- 2025-11-02 (f5414219) pitfall: the write-back hazard that stores a slot's old
  ...
  value where it already was (code).
...
- 2025-10-26 (d487c901) measurement: the score-based eviction order keeps the
  ...
  data in [[window-manager.fact/window-pipeline-savings]] (sourced).
...
- 2025-09-28 (a0daf858) rationale: the lowerer tracks which value occupies each
  ...
  register file on every use (code).
...
- 2025-09-29 (4c13a489) rationale: the per-access register load/store stopped
  ...
  error construction from the hot loop (code).
...
- 2025-10-15 (0d24ab09) rationale: the raw register-file pointer is reserved for
  a future optimization that did not land in this window (uncertain).

- 2025-10-08 (303bf064) rationale: the hot register window was fixed at three
  slots, reduced from four; the fusion budget check was retuned from 4 to 3 to
  match (code).

- 2025-10-08 (303bf064) rationale: three slots is the knee of a
  Sethi-Ullman-coverage vs shuffle-handler tradeoff — 2 regs cover ~60% of
  expression trees, 3 cover ~70-85% needing only three swap handlers (any 3!
  permutation reachable in <=2 swaps), while 4 regs give a marginal coverage gain
  but double the swap handlers and 6 explode to fifteen (sourced).
...
- 2025-10-14 (b493e78a) pitfall: a mul feeding the left of an add/sub must not be
  ...
  the fused form; the matcher refuses the fusion when the RHS is also a Mul (code).
...
- 2025-10-14 (b493e78a) rationale: the matched MADD/MSUB pattern is not emitted as
  ...
  (no trampoline FMA handler exists yet) (code).
...
- 2025-10-14 (b493e78a) pitfall: when a fused two-binary op has the same value as
  ...
  without disturbing the other (code).
...
- 2025-10-14 (b493e78a) statement: the fused two-binary lowering originally
  ...
  internal error naming both (code).
...
- 2025-10-13 (82b6303a) replaced [[raw-ptr-regfile]]: raw-pointer register access
  ...
  with the raw-pointer handler argument retired to dead weight (code).
