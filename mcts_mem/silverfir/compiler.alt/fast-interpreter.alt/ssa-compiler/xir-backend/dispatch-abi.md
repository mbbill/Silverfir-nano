...
- 2025-10-08 (e95a9e38) rationale: the entry point uses the ordinary C calling
  ...
  once the terminal handler unwinds (code).
...
- 2025-10-08 (303bf064) rationale: dispatch combines `musttail` (forces JMP rather
  ...
  registers across the whole tail-chain instead of spilling between handlers (code).
...
- 2025-10-08 (55c52766) rationale: the function result is recovered by reading the
  ...
  terminal handler just unwinds with no write-back (code).

- 2025-10-20 (bb0d2820) rationale: under the `musttail` + `preserve_none`
  tail-chained dispatch what changed is the register model, not the dispatch
  mechanism (code).

- 2025-10-20 (bb0d2820) statement: the `musttail` + `preserve_none` tail-chained
...
- 2026-02-08 (da03882b) rationale: the Return handler unconditionally refreshes
  ...
  always-refresh cost is an O(1) lookup (code).
...
- 2025-10-11 (d9c0ccce) pitfall: the first cached memory-0 base/length was a raw
  ...
  that first led to dropping the cache entirely (code).
...
- 2025-10-13 (10a69247) dropped: the per-execution mem0 base+size pointer cache —
  ...
  memory op; loads/stores now always go through the store's safe borrow (code).
...
- 2025-09-17 (e85e902d) replaced [[regs-base-pointer-abi]]: a regs_base
  ...
  set staying in registers with zero prologue/epilogue between handlers (code).
...
- 2026-02-07 (88b3fda7) replaced [[mem0-threaded-params]]: the XIR handler ABI
  ...
  writing back through the parameter pointers (code).
