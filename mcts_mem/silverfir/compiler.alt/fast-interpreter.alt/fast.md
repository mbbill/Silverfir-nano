...
- 2025-08-09 (65015cb5) rationale: the fast backend exists to maximize
  ...
  execution (stated in the accompanying FAST_INTERPRETER_PLAN.md) (sourced).
...
- 2025-08-11 (4bab9be0) dependency: the threaded form pulls the first C code
  ...
  optional glue but load-bearing to the design (code).
...
- 2025-08-14 (53e8efd5) statement: function evaluation switched from running
  ...
  interpreter was retained but no longer reached (code).
...
- 2025-08-09 (f8283349) statement: although the fast backend is the library
  ...
  on; the fast path traps rather than silently miscomputing on any
  opcode it does not yet support (code).

- 2025-08-09 (f8283349) statement: forcing the baseline interpreter on in the
  spec-test and WASI-test harnesses is evidence that the fast path was not yet
  trusted for full correctness at this stage (uncertain).
...
- 2025-08-15 (d3e9a056) rationale: a null funcref/externref is represented on the
  ...
  tests against usize::MAX accordingly (code).
