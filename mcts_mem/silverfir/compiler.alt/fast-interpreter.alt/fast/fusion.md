...
- 2025-12-06 (b7b5dc6a) rationale: build-time super-instruction fusion was
  ...
  dominant cost again (code).

- 2025-12-06 (b7b5dc6a) rationale: fusion pays off when dispatch count dominates,
  not when it only collapses copies a smarter operand model can already elide
  (sourced).
...
- 2025-12-13 (a2b353da) rationale: which adjacent-opcode sequences become
  ...
  hot sequences in real workloads rather than guessed (code).
...
- 2025-12-15 (a258d56c) rationale: fusion exists to cut dispatch count — each
  ...
  times the run's frequency (code).
...
- 2025-12-15 (10fc8467) statement: fusion candidates exclude sequences with
  ...
  mid-sequence control flow (code).
...
- 2026-02-06 (701894ef) rationale: the fusion code generator stopped hand-listing
  ...
  because the semantic macros now cover the whole op set (code).
...
- 2026-01-25 (1dad15de) rationale: fused handlers reuse the single-opcode
  ...
  positions aliases them to the same TOS register (read-before-write) (code).
...
- 2026-06-14 rationale: fusion is not unconditionally good. Its discovered
  ...
  small size or workload-agnostic performance is the goal (sourced).
