...
- 2026-03-15 (5828c3c2) statement: correction to the 2026-03-15 (b9b02d80) entry
  ...
  step on top of the implemented typed pipeline, not a substitute for it (sourced).
...
- 2026-03-14 (7b489b53) statement: the fixed machine roles (ctx, fp, and pinned
  ...
  helper-backed boundaries reload it after return (sourced).

## Moves

- 2026-03-13 (110b77f0) replaced [[general-scratch-registers]]: memory 0 is the hottest runtime view, so the two fixed scratch slots are repurposed to pin mem0_base/mem0_size for the whole function; ad hoc scratch is instead borrowed from transient lanes that no live value currently owns (code).

- 2026-04-04 (ea0cf447) replaced [[static-partitioned-register-file]]: a register fixed as cache-only or transient-only by its number could not be reassigned to whichever purpose a block needed; making each dynamic register's role explicit owner metadata lets one bank serve cached locals and transient stack values per program point (code).
