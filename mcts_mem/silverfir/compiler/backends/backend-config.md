...
- 2026-03-15 (81cd64b0) rationale: the target-ABI and physical-register-mapping
  ...
  `abi.rs` as the single source of truth, separate from the tunable per-bank
  budget chosen in `config.rs` (code).

- 2026-03-15 (81cd64b0) rationale: the target-ABI facts were deliberately
  insulated from the tunable per-bank budget so changing the physical mapping or
  save/restore layout could not silently change compiler policy and tuning the
  budget needed no ABI edit (sourced).
...
- 2026-03-24 (413275ef) statement: the register-budget preset
  ...
  and transient lane counts), now selected from inside abi.rs (code).
...
- 2026-03-27 (12ef375a) rationale: a backend's GP transient budget must be at
  ...
  with a debug_assert catching under-provisioned budgets at construction (code).
...
- 2026-05-14 (f54f5dcf) measurement: arm64 widened its local-call GP
  ...
  lane count so not every volatile lane is forced to be an argument lane (code).
...
- 2026-03-15 (21ed5413) statement: the armv7a backend is the first non-arm64,
  ...
  than encoding arm64 assumptions (code).
