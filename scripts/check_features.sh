#!/usr/bin/env bash
#
# check_features.sh — build-check sf-nano-core against a matrix of feature
# combinations in both debug and release profiles, then cross-check the
# dependent crates.
#
# What it does
# ------------
# 1. `cargo check -p sf-nano-core` with each combination below, in both dev
#    and release profiles. Counts warnings and errors; reports both.
# 2. `cargo check --workspace` with default features (sanity check that the
#    binaries still build against the default feature set).
# 3. `cargo check` in `sf-nano-cli-minimal` (excluded from workspace).
#
...
# sf-nano-core are not supported today (no execution backend).
#
...
COMBOS=(
    ...
    # jit alone (the minimum supported configuration)
    "only-jit|jit"

    # jit + one other
    "jit+wasi|jit,wasi"
    "jit+validator|jit,validator"
    "jit+call-trace|jit,call-trace"
    "jit+guard-pages|jit,guard-pages"
    "jit+ir-dump|jit,ir-dump"
    "jit+jitdump|jit,jitdump"

    # Realistic build configurations
...
run_one() {
    local label="$1" profile="$2" spec="$3"
    local fflags
    ...
    local log="$LOG_DIR/${label}-${profile}.log"

    # Intentional word splitting on $fflags/$profile_flag (plain strings,
    # controlled input).
    # shellcheck disable=SC2086
    if RUSTFLAGS="$rustflags" cargo check -p sf-nano-core \
            $profile_flag $fflags \
            >"$log" 2>&1; then
        local wc_local
...
echo "=== sf-nano-core feature matrix ==="
echo "  label                  profile   status"
echo "  ---------------------- --------  -------"
