...
#
# The target triple is still armv7-unknown-linux-musleabihf — the JIT
# itself emits Thumb-2 via `enc_t2.rs` (gated by `sf_arm32_isa_thumb`,
# which build.rs sets when the `thumb2-test` cargo feature is on). The
# resulting binary is A32 Rust runtime + Thumb-2 JIT code, bridged by
# ARM/Thumb interworking at the BX/BLX boundary. cortex-a15 under
# qemu-arm-static executes both modes happily.
#
# Usage:
#   ./scripts/armv7m-run-tests.sh [--full] [--debug]
#
...
STATE_DIR="$(mktemp -d /tmp/armv7m-run-tests.XXXXXX)"
WRAPPER=""
...
cleanup() {
    ...
    if [[ "${COLIMA_STARTED:-}" = "1" ]]; then
        echo "[armv7m-run-tests] Stopping Colima (we started it)..."
...
COLIMA_STARTED=0
if ! colima status &>/dev/null; then
    echo "[armv7m-run-tests] Starting Colima..."
    colima start
    COLIMA_STARTED=1
fi

if ! colima ssh -- which qemu-arm-static &>/dev/null; then
    echo "[armv7m-run-tests] Installing qemu-user-static in Colima VM..."
...
echo "[armv7m-run-tests] Cross-compiling CLI ($PROFILE, Thumb-2 JIT) for ARMv7..."
cargo build --target "$TARGET" -p sf-nano-cli --no-default-features --features jit,thumb2-test \
...
# input) flows through unchanged. See the armv7a sibling script for the
# rationale; the same constraint applies here.
cli_bin="$1"
shift
...
if [[ "$FAST_RUN" -eq 1 ]]; then
    FAST_FLAG="--fast"
    echo "[armv7m-run-tests] Running benchmarks under QEMU ($PROFILE, Thumb-2, fast)..."
else
    echo "[armv7m-run-tests] Running benchmarks under QEMU ($PROFILE, Thumb-2, full)..."
fi
echo
