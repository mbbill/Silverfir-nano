...
The CLI's `memprof` feature records allocation totals and compiler phases,
...
The report contains live and peak heap bytes, allocation/reallocation counts,
executable-buffer capacity, guard-memory committed bytes, and compiler-phase
durations and allocation traffic. Traffic counts new blocks and positive realloc
growth. Phase traffic is process-wide, so nested and parallel spans overlap and
must not be summed. At most 16,384 phase records are retained; the report shows
the number omitted. These allocator counts are not RSS.

Containers remain ordinary `alloc` types under every feature combination.
The CLI installs `TrackingAllocator`; merely enabling the core feature does not
replace an embedder's allocator. Profiler bookkeeping and allocations preceding
the measurement are excluded. Rust container-type attribution, allocation
backtraces and the old timeline viewer have been removed to keep this internal
tool independent of runtime ownership and public API design. The report is
self-contained HTML generated by `sf-nano-memprof-report`.
