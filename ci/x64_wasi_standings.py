...
Diagnostic tooling for .github/workflows/x64-standings.yml: drives
...
a snapshot instrument. Reverse order in a second draw checks order sensitivity.
"""
...
def run_v8(node: str, seconds: float):
    ...
    failures: list[str] = []
    if proc.returncode:
        failures.append(f"node exited {proc.returncode}: {proc.stderr[-1000:]}")
    in_table = False
...
def main(argv: list[str] | None = None) -> int:
    ...
    engines = ["silverfir-nano.jit", "wasmtime.cranelift", "v8"]
    if args.reverse == "true":
        engines.reverse()
    for engine in engines:
        print(f"== {engine} ==", flush=True)
        if engine == "v8":
            rates, failed = run_v8(args.node, args.time)
        elif engine == "wasmtime.cranelift":
            rates, failed = run_native(run_tests, wasmtime, ("run", "--dir", "."), args.time)
        else:
            rates, failed = run_native(run_tests, nano, (), args.time)
        runtimes[engine] = rates
        failures += [f"{engine} {entry}" for entry in failed]
    args.note.append("engine order: " + ", ".join(engines))
    # A partial field must never turn a smaller denominator into a better rank.
    for engine, rates in runtimes.items():
        if len(rates) != 17:
            failures.append(f"{engine}: expected 17 metrics, found {len(rates)}")
        if set(rates) != set(runtimes["silverfir-nano.jit"]):
            failures.append(f"{engine}: metric field differs from nano")
        if any(not math.isfinite(value) or value <= 0 for value, unit in rates.values()):
            failures.append(f"{engine}: non-positive or non-finite metric")
